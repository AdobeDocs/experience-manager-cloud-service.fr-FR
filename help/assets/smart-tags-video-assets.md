...
Les balises actives appliquées sont triées par ordre décroissant de note [de](#confidence-score-video-tag)confiance, combinées pour les balises d’objet et d’action, dans les balises dynamiques.
...
Les ressources vidéo existantes dans la gestion des actifs numériques ne sont pas balisées automatiquement. Vous devez [!UICONTROL retraiter les ressources] manuellement pour générer des balises actives à leur intention.
...
1. Sélectionnez le [!DNL Adobe Experience Manager] logo, puis sélectionnez des ressources dans la page [!UICONTROL Navigation] .
...
1. Sélectionnez l’icône ![Retraiter les ressources](assets/do-not-localize/reprocess-assets-icon.png) [!UICONTROL Retraiter les ressources] et sélectionnez l’option Processus  complet.

![Retraiter les ressources pour ajouter des balises aux vidéos du référentiel DAM existant](assets/reprocess.gif)

Une fois le processus terminé, accédez à la page [!UICONTROL Propriétés] de toute ressource vidéo du dossier. Les balises ajoutées automatiquement sont affichées dans la section Balises  dynamiques de l’onglet [!UICONTROL Simple] . Ces balises actives appliquées sont triées par ordre décroissant de note [de](#confidence-score-video-tag)confiance.
...
1. Sélectionnez l&#39;icône ![de recherche de l&#39;icône](assets/do-not-localize/search_icon.png) de recherche pour afficher le champ Omnisearch.
...
Pour en savoir plus sur la manière de modérer les balises actives pour les ressources, voir [Gestion des balises](smart-tags.md#manage-smart-tags-and-searches)actives.

![Modération des balises actives vidéo](assets/manage-video-smart-tags.png)

>[!NOTE]
>
>Les balises modérées à l’aide des étapes de la section [Gérer les balises](smart-tags.md#manage-smart-tags-and-searches) actives ne sont pas mémorisées lors du retraitement de la ressource. Le jeu de balises d’origine s’affiche à nouveau.
...
1. Ouvrez l’onglet Traitement [!UICONTROL des] fichiers dans [!UICONTROL Propriétés]du dossier.

1. Dans le menu [!UICONTROL Balises dynamiques pour les vidéos] , l’option [!UICONTROL Héritée] est sélectionnée par défaut et la balise active vidéo.

   Lorsque l’option [!UICONTROL Héritée] est sélectionnée, le chemin d’accès au dossier hérité est également visible, ainsi que les informations indiquant s’il est défini sur [!UICONTROL Activer] ou [!UICONTROL Désactiver].

   ![Désactivation du balisage dynamique vidéo](assets/disable-video-tagging.png)

1. Sélectionnez [!UICONTROL Désactiver] pour opt-out de balisage intelligent des vidéos téléchargées dans le dossier.

>[!IMPORTANT]
>
>Si vous avez choisi de ne pas baliser les vidéos sur un dossier au moment du téléchargement et que vous souhaitez marquer les vidéos de manière intelligente après le téléchargement, **[!UICONTROL activez l’option]** Activer les balises actives pour les vidéos [!UICONTROL à partir de l’onglet Traitement] des [!UICONTROL ressources du dossier] Propriétés [[!UICONTROL et utilisez l’optionRetraiter les ressources pour ajouter des balises actives à la vidéo.]](#smart-tag-existing-videos)
...
Pour ajouter la configuration OSGI du score de confiance au projet déployé en tant que Cloud Service [!DNL Adobe Experience Manager] via Cloud Manager :

* Dans le [!DNL Adobe Experience Manager] projet (`ui.config` depuis l&#39;Archétype 24, ou précédemment `ui.apps`), la configuration `config.author` OSGi, incluez un fichier de configuration nommé `com.adobe.cq.assetcompute.impl.senseisdk.SenseiSdkImpl.cfg.json` avec le contenu suivant :
