...
* Certains utilisateurs peuvent rencontrer des problèmes lors de la création de formulaires sur `Cloud Service version 2024.5.16461`, le problème est que, lorsqu’un utilisateur crée un formulaire, le message d’erreur suivant s’affiche dans la boîte de dialogue de création :

  `A server error occurred. Try again after sometime.`

  Le problème se produit car l’auteur publie le formulaire sans publier au préalable le modèle utilisé dans le formulaire, ce qui entraîne l’ajout de la variable `jcr:uuid` et d’autres propriétés protégées et générées par le système à la propriété `<template-path>/initial/jcr:content` noeud . Cela entraîne des échecs lors de la création ultérieure du formulaire.

  Suivez les étapes ci-dessous pour résoudre le problème :

   1. Assurez-vous que le modèle que vous utilisez dans votre formulaire ne comporte pas la variable `jcr:uuid` et d’autres propriétés protégées générées par le système sur le chemin d’accès `<template-path>/initial/jcr:content` noeud .
