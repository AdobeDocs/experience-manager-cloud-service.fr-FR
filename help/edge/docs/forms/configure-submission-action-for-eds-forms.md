---
title: Configuration des actions Envoyer pour AEM Forms avec Edge Delivery Services
...
Une fois qu’un utilisateur a cliqué sur **envoyer** dans votre formulaire, vous devez indiquer à Edge Delivery Services ce qu’il doit faire de ces données. Vous disposez de deux options principales :

## Méthode 1 : utilisation du service de soumission AEM Forms (simplifié)

Ce service est idéal pour les actions courantes et simples comme l’envoi de données à une feuille de calcul ou un e-mail.

**Qu&#39;est-ce que c&#39;est et comment peut-il vous aider ?**

Le service d’envoi [Forms](/help/forms/forms-submission-service.md) est un point d’entrée hébergé par Adobe. Lorsque votre formulaire lui envoie des données, ce service prend le relais et effectue une action préconfigurée. Il est conçu pour être facile à configurer. Vous pouvez configurer : Envoi aux feuilles de calcul ou à l’e-mail :

* **Envoyer à une feuille de calcul :** ajoutez automatiquement les données de formulaire envoyées sous la forme d’une nouvelle ligne dans une feuille Google ou un fichier Microsoft Excel (stocké sur OneDrive ou SharePoint).
...
* **Accès aux feuilles de calcul :** pour envoyer des données à une feuille Google ou à un fichier Excel sur OneDrive/SharePoint, le compte de service Adobe (souvent `forms@adobe.com`) a généralement besoin d’une autorisation **de modification** sur cette feuille de calcul.
...
**Organigramme Du Service D’Envoi De Forms**
<!--
```mermaid
    graph TD
...
Pour les besoins plus complexes, les formulaires [ (en particulier ceux créés avec l’éditeur universel) peuvent envoyer directement des données à votre instance de publication AEM as a Cloud Service](/help/forms/configure-submit-actions-core-components.md). Cela permet de déverrouiller AEM pour optimiser la puissance du serveur principal.

**À quel moment faut-il procéder à l’envoi au service de publication AEM ?**

* Pour déclencher des workflows AEM personnalisés après envoi.
* Pour utiliser le modèle de données de formulaire (FDM) AEM à intégrer à des bases de données ou à d’autres systèmes d’entreprise.
* Pour vous connecter à des services tiers tels que Marketo, Microsoft Power Automate ou Adobe Workfront Fusion.
* Pour stocker des données à des emplacements spécifiques tels que le stockage Blob Azure ou des listes/bibliothèques de documents SharePoint (pas seulement de simples feuilles de calcul).
* Lorsque vous disposez d’une logique complexe de validation côté serveur ou de traitement des données dans AEM.

**Actions Envoyer disponibles (envois de publication AEM)**

* [Envoyer vers un point d’entrée REST](/help/forms/configure-submit-action-restpoint.md)
...
* [Envoyer à OneDrive (en tant que documents)](/help/forms/configure-submit-action-onedrive.md)
...
>[!NOTE]
>
> Même si le ciblage d’une feuille Google/d’Excel à partir de l’instance de publication AEM implique des étapes de configuration différentes de celles du service d’envoi direct de Forms.

**Organigramme d’envoi de publication AEM**
...
![Organigramme d’envoi de publication AEM](/help/forms/assets/eds-aem-publish.png)
Ce diagramme de flux présente un formulaire envoyé au service de publication AEM, qui gère ensuite des tâches complexes en arrière-plan.

### Forms Submission Service et AEM Publish Submissions

| Fonctionnalité | Service de soumission Forms | Envois de publication AEM |
| :- | :- | :-- |
...
| **Création de formulaires** | Idéal pour les formulaires basés sur des documents ; OK pour les formulaires UE simples | Idéal pour les formulaires créés dans l’éditeur universel |
| **Effort de configuration** | Faible (configuration souvent simple) | Supérieur (nécessite la publication AEM, Dispatcher, OSGi, configuration du réseau CDN) |
| **Système principal** | service hébergé sur Adobe | Votre instance de publication AEM as a Cloud Service |
| **Flexibilité** | Limité à la feuille/l’e-mail | Très flexible, gamme complète d’actions AEM Forms |
| **Exemple** | Transférer les données de formulaire vers une feuille de Google | Demande de prêt déclenchant un workflow d’approbation AEM |

## Intégration de Forms à différents sites ou pages
...
### Pourquoi incorporer un formulaire ?

* Vous disposez d’un formulaire « Contactez-nous » standard créé avec l’éditeur universel qui doit apparaître sur plusieurs pages de destination créées dans le cadre de la création basée sur des documents.
* Le contenu principal de votre site web se trouve dans Document Authoring (DA) et vous devez inclure un formulaire spécialisé.
* Vous souhaitez réutiliser un seul formulaire bien entretenu dans plusieurs projets EDS différents.

### Fonctionnement Technique De L’Incorporation De Formulaires

La page sur laquelle vous souhaitez que le formulaire apparaisse (appelons-le « Page hôte ») contient du code (généralement un bloc ou un script spécial). Lorsqu’un utilisateur visite la page hôte, ce code envoie une requête à l’URL où le formulaire réel est hébergé (appelons-le « Source de formulaire »). Le Source de formulaire renvoie ensuite son HTML, que la page hôte injecte et affiche.
...
## Configuration de CORS pour Embedded Forms

[Le partage de ressources entre origines multiples (CORS))](https://experienceleague.adobe.com/fr/docs/experience-manager-learn/foundation/security/understand-cross-origin-resource-sharing) est une fonctionnalité de sécurité du navigateur. Si votre page hôte (par exemple, `site-a.com`) tente de récupérer un formulaire d’un autre domaine (par exemple, `forms-site-b.com`), le navigateur le bloque, sauf si `forms-site-b.com` l’autorise explicitement via des en-têtes CORS.

Sans en-têtes CORS corrects sur le serveur **Form Source**, le navigateur empêche la page hôte de charger le formulaire, et le formulaire incorporé n’apparaîtrait pas.

### Comment configurer CORS sur le site qui diffuse votre formulaire ?

Vous devez configurer le serveur hébergeant le **Form Source** pour envoyer des en-têtes HTTP spécifiques dans sa réponse. La méthode exacte dépend de votre configuration EDS (par exemple, pour les projets Franklin, cela est souvent fait dans un fichier de configuration `helix-config.yaml` ou similaire dans votre référentiel GitHub qui contrôle le comportement du réseau CDN ou la logique du programme de travail Edge).
En-têtes clés à ajouter aux réponses de Source de formulaire :
...
* `Access-Control-Allow-Methods: GET, OPTIONS` (vous aurez peut-être besoin de `POST` si l’envoi du formulaire lui-même est également d’origine croisée, mais les envois sont généralement envoyés à un point d’entrée distinct, souvent de même origine ou spécifiquement configuré).
...
* **Règles de réseau CDN :** votre réseau CDN peut vous offrir des moyens de traiter les requêtes en proxy. Par exemple, une requête à `host-page.com/embedded-form` peut être routée en interne par le réseau CDN pour récupérer du contenu de `form-source.com/actual-form`, ce qui lui donne l’apparence de la même origine au navigateur. La configuration de cette fonctionnalité peut être complexe.
* **Bases de code multiples (Helix 4) :** si votre page hôte et votre Source de formulaire se trouvent dans des référentiels GitHub différents (communs dans les configurations Helix 4), assurez-vous que tout « bloc de formulaire » de JavaScript nécessaire pour générer ou gérer le formulaire est disponible dans la base de code de la page hôte ou que l’HTML de formulaire récupéré à partir de la Source de formulaire est autonome avec tous ses JavaScript nécessaires. Les documents originaux mentionnent que pour « helix4 avec des codebases différentes, alors vous devez ajouter le bloc Form sur les deux codebases ».

### Étapes courantes de configuration de l’architecture

Voici quelques méthodes courantes que vous pouvez utiliser pour configurer vos formulaires, en combinant des méthodes de création à des stratégies d’envoi, ainsi que des points de configuration clés.

#### Formulaire basé sur des documents avec envoi de feuille de calcul/e-mail

Il s’agit de la configuration la plus simple. Vous créez votre formulaire dans Word/Google Docs et il envoie des données à une feuille de calcul ou à un e-mail via le service d’envoi de Forms.

1. Définissez votre formulaire dans un document Word/Google à l’aide de la structure de tableau ou du bloc de formulaire spécifié.
...
1. Si vous utilisez des feuilles de calcul, assurez-`forms@adobe.com` que vous disposez d&#39;un accès en modification.
1. Publiez la page contenant le formulaire d’AEM sur votre site Edge Delivery.

   **Éditeur universel + Architecture du service d’envoi de Forms**

   ![Éditeur universel + Architecture du service d’envoi de Forms](/help/forms/assets/eds-ue-fss.png)
...
Cette configuration utilise l’éditeur universel pour la création de formulaires et votre instance de publication AEM pour un traitement back-end puissant (workflows, FDM, etc.). Cela nécessite davantage de configuration.

1. **Créer un formulaire dans UE :** créez votre formulaire dans l’éditeur universel. Configurez son action d’envoi pour pointer vers une action AEM Forms (par exemple, « Appeler un workflow AEM », « Envoyer à l’aide du modèle de données de formulaire »).
1. **Configuration d’AEM Dispatcher (sur votre niveau de publication AEM) :**
   * **Aucune redirection :** assurez-vous que vos règles Dispatcher ne *pas* redirigent les requêtes effectuées vers des chemins d’accès `/adobe/forms/af/submit/...`.
   * **Autoriser les envois :** modifiez vos filtres Dispatcher (par exemple, dans `filters.any`) pour `allow` explicitement les requêtes POST aux `/adobe/forms/af/submit/...` à partir du domaine ou des adresses IP de votre site Edge Delivery.
...
1. **Règle de redirection CDN (sur votre réseau CDN Edge Delivery) :**
   ...
   * Lors de l’envoi du formulaire sur la page EDS, il se peut qu’il cible un chemin relatif tel que `/adobe/forms/af/submit/...`. Vous avez besoin d’une règle sur votre réseau CDN Edge Delivery (ou programme de travail Edge) qui indique : « Si une demande arrive en `your-eds-domain.hlx.page/adobe/forms/af/submit/...`, transférez-la (par proxy ou par redirection) vers `your-aem-publish-instance.com/adobe/forms/af/submit/...`. »
...
Le flux s’affiche : l’utilisateur effectue des envois sur le site EDS, le réseau CDN achemine vers AEM Dispatcher, puis AEM Publish le traite.

#### Incorporation d’un formulaire dans une page de création de documents

Le contenu principal de votre site web est créé dans Document Authoring (DA). Vous créez votre formulaire à l’aide de la création basée sur des documents ou de l’éditeur universel séparément, puis vous l’incorporez dans votre page de données.
...
1. **Créer et publier le formulaire :**
   ...
   * Configurez sa méthode d’envoi (vers le service d’envoi Forms ou l’instance de publication AEM, conformément aux configurations 1, 2 ou 3).
...
1. **Créer une page dans DA :** permet de créer ou de modifier une page dans Document Authoring.
...
1. **Publier la page DA :** publiez votre page DA. Il va maintenant récupérer et afficher le formulaire.

   **Forms incorporé dans l&#39;architecture DA**

   ![Forms incorporé dans l&#39;architecture DA](/help/forms/assets/eds-forms-embedd-da.png)

   Une page DA extrayant un formulaire à partir d’un autre emplacement EDS s’affiche. Le formulaire incorporé gère son propre envoi.

## Résolution des problèmes

* **Mon envoi de formulaire ne fonctionne pas.**
   * **Vérification des erreurs de console :** ouvrez la console de développement de votre navigateur (généralement F12) et recherchez les erreurs dans l’onglet Réseau ou l’onglet Console lors de l’envoi.
   * **Vérifier l’URL d’envoi :** le formulaire tente-t-il d’être envoyé au point d’entrée correct (URL du service d’envoi Forms ou chemin de publication AEM) ?
   * **Service d&#39;envoi Forms :** en cas d&#39;envoi vers une feuille de calcul, `forms@adobe.com` a-t-il reçu un accès en modification ? L’URL de la feuille de calcul est-elle correcte ?
   * **Envois de publication AEM:**
      * Votre Dispatcher autorise-t-elle les publications à `/adobe/forms/af/submit/...` ?
      * Le filtre du référent Sling sur l’instance de publication AEM est-il configuré pour autoriser votre domaine EDS ?
      * Vos règles de redirection CDN pour `/adobe/forms/af/submit/...` fonctionnent-elles correctement ?

* **Mon formulaire incorporé n’apparaît pas.**

   * **CORS !** Il s’agit de la raison la plus courante. Recherchez les erreurs CORS dans la console du navigateur. Assurez-vous que le site *hébergement* le formulaire comporte les en-têtes `Access-Control-Allow-Origin` corrects.
   * **URL du formulaire correcte ?** Le code incorporé sur la page hôte pointe-t-il vers l’URL active correcte du formulaire ?
