...
description: Prise en charge Oauth2 du service de messagerie dans Adobe Experience Manager as a Cloud Service
...
# Prise en charge OAuth2 du service de messagerie {#oauth2-support-for-the-mail-service}
...
Pour plus d’informations sur AEM as a Cloud Service Mail Service, voir [Envoi d’email](/help/implementing/developing/introduction/development-guidelines.md#sending-email).
...
1. Sélectionnez les autorisations ci-dessous pour votre application, puis cliquez sur **Ajouter une autorisation** :
   ...
   * `openid`
   * `offline_access`
1. Accédez à **Authentification** - **Ajoutez une plateforme** - **Web**, et dans la section **URL de redirection**, ajoutez les URL ci-dessous, une avec et une sans barre oblique :
   * `http://localhost/`
   * `http://localhost`
...
1. Autoriser l’autorisation si demandé
...
1. Générez le accessToken à l&#39;aide du refreshToken produit lors de la procédure précédente. Pour ce faire, utilisez l’URL suivante, en remplaçant les valeurs de `<client_id>`,`<client_secret>` et `<refreshToken>` :
...
1. Envoyez un courrier électronique à l’aide de accessToken pour vérifier si fonctionne correctement.

>[!NOTE]
>
> Vous pouvez obtenir la collection d’API Postman à partir de [cet emplacement](https://docs.microsoft.com/en-us/azure/active-directory/develop/v2-oauth2-auth-code-flow).

### Intégration à AEM en tant que Cloud Service {#integration-with-aem-as-a-cloud-service}
...
1. Ajoutez les étendues suivantes à la configuration :
   * `openid`
   * `offline_access`
   * `https://outlook.office365.com/Mail.Send`
   * `https://outlook.office365.com/Mail.Read`
   * `https://outlook.office365.com/SMTP.Send`
1. Créez un fichier de propriétés OSGI `called com.day.cq.mailer.impl.DefaultMailService.cfg.json`
under 
...
Si le service de messagerie ne fonctionne pas correctement, vous devrez, dans la plupart des cas, régénérer la balise `refreshToken` comme décrit ci-dessus, en transmettant la nouvelle valeur via l’API Cloud Manager. Le déploiement de la nouvelle valeur prend quelques minutes.
