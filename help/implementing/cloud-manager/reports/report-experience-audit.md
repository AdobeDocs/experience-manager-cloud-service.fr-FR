...
Les résultats sont informatifs et permettent au responsable de déploiement de voir les scores et les différences existant entre les scores précédents et actuels. Cette insight est utile pour déterminer s’il existe une régression introduite par le déploiement actuel.

[Google Lighthouse](https://developer.chrome.com/docs/lighthouse/overview/), un outil open source de Google, alimente le contrôle de l’expérience et Cloud Manager l’active dans tous les pipelines de production.
...
Le contrôle de l’expérience est disponible par défaut pour les pipelines de production. Il peut être activé de manière facultative pour les pipelines full-stack et front-end. Dans tous les cas, vous devez définir les chemins de contenu à évaluer lors de l’exécution du pipeline.

1. Selon le type de pipeline que vous souhaitez configurer, effectuez l’une des opérations suivantes :

   * Pour définir les chemins que l’audit doit évaluer, [ajoutez un pipeline de production](/help/implementing/cloud-manager/configuring-pipelines/configuring-production-pipelines.md).
   * [Ajoutez un pipeline hors production](/help/implementing/cloud-manager/configuring-pipelines/configuring-non-production-pipelines.md), si vous souhaitez activer l’audit sur un pipeline front-end ou full-stack.
...
1. Pour utiliser le contrôle de l’expérience lors de l’ajout ou de la modification d’un pipeline hors production, cochez la case **Contrôle de l’expérience**. Cette option est disponible dans l&#39;onglet **Code**.
...
1. Pour les pipelines de production et hors production, vous définissez les chemins d’accès à inclure dans le contrôle de l’expérience dans l’onglet **Contrôle de l’expérience**.
   ...
   * Par exemple, si votre site est `wknd.site` et que vous incluez des `https://wknd.site/us/en/about-us.html` (exemple d’URL uniquement) dans le contrôle de l’expérience, saisissez le chemin d’accès `/us/en/about-us.html`.
...
1. Continuez à ajouter des chemins selon vos besoins en répétant les deux étapes précédentes.
   ...
   * Si vous ne définissez aucun chemin, le contrôle de l’expérience inclut par défaut la page d’accueil du site.
...
Pour ouvrir la boîte de dialogue **5 pages les plus lentes**, cliquez sur **Afficher les pages les plus lentes**. Les cinq pages les moins performantes que vous [avez configurées pour l’audit](#configuration) s’affichent.
...
Si vous cliquez sur le graphique à un moment donné, un pop-up s’ouvre avec les détails de cette analyse. Cliquez sur le **ouvrir l’analyse de contrôle de l’expérience** pour charger ces résultats d’analyse dans la section **[Résultats de l’analyse du contrôle de l’expérience](#scan-results)**.
...
La section **Résultats de l’analyse du contrôle de l’expérience** donne des détails sur les scores de toutes les pages numérisées. Utilisez les boutons **Précédent** et **Suivant** pour parcourir les résultats et choisir le nombre d’affichages à afficher.
...
En plus d’être exécutés pendant l’exécution du pipeline, les rapports de contrôle de l’expérience peuvent également être générés à la demande. Cette option est une solution efficace pour analyser rapidement vos pages, sans avoir à exécuter un pipeline.
...
Le pipeline affiche une section d’erreur extensible pour afficher les chemins d’URL relatifs auxquels il ne peut pas accéder.
...
Voici quelques raisons pour lesquelles les pages ne sont pas disponibles :
...
>[!TIP]
>
>[L’accès aux rapports bruts](#scan-results) d’une page fournit des détails sur les raisons pour lesquelles la page n’a pas été contrôlée.
