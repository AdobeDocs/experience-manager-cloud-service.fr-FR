...
La date de publication de Cloud Manager dans AEM as a Cloud Service version 2021.8.0 est le 12 août 2021.
...
* Fonction de libre-service pour permettre aux utilisateurs de créer et de gérer plusieurs référentiels via l’interface utilisateur de Cloud Manager.
...
* Parfois, lorsqu’un pipeline est déclenché deux fois pour une raison quelconque, l’une ou l’autre des exécutions échoue avec une erreur *`cannot update pipeline execution status`*.
