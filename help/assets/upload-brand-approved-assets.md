...
### Quels types de ressources puis-je charger vers AEM Assets Content Hub et d’où ? {#asset-types-upload-to-content-hub}

Les utilisateurs d’AEM Assets Content Hub autorisés à ajouter des ressources peuvent charger des ressources approuvées par la marque à partir de leur système de fichiers local. Toutes les ressources chargées, quelle que soit leur structure de dossiers d’origine, sont affichées au niveau supérieur dans Content Hub afin d’améliorer les fonctionnalités de recherche.

### Comment AEM Assets Content Hub améliore-t-il la recherche et l’organisation des ressources ? {#search-content-hub}

AEM Assets Content Hub améliore la recherche et l’organisation des ressources en permettant aux utilisateurs de définir des détails importants pour chaque chargement, tels que le nom de la campagne, les mots-clés, les canaux, la période et la région. Il génère également automatiquement des propriétés supplémentaires pour chaque ressource (telles que la taille, le format et la résolution du fichier) et utilise Adobe AI pour appliquer des balises intelligentes, ce qui facilite et accélère la recherche des ressources appropriées.

### Comment charger des ressources de mon système de fichiers local vers AEM Assets Content Hub ? {#upload-assets-content-hub}

Pour charger des ressources de votre système de fichiers local vers AEM Assets Content Hub, cliquez sur **Ajouter Assets** pour ouvrir la boîte de dialogue de chargement. Vous pouvez faire un glisser-déposer de fichiers ou de dossiers, ou parcourir manuellement pour les sélectionner. Vous devez regrouper vos ressources sous un nom de campagne et il est recommandé de remplir d’autres champs tels que mots-clés, canaux, période et région pour une meilleure organisation. Une fois prêt, cliquez sur **Charger**, vérifiez les détails et confirmez pour commencer le chargement.

### Comment fonctionne le processus d’approbation des ressources dans AEM Assets Content Hub ? {#asset-approval-content-hub}

Si le bouton d’approbation automatique est activé, les ressources chargées à l’aide d’AEM Assets Content Hub sont automatiquement disponibles. Si elle est désactivée, les ressources chargées sont placées dans le dossier **ressources-hydratées** d’Assets as a Cloud Service et vous devez modifier manuellement en bloc leur statut en **Approuvé** pour qu’elles s’affichent dans Content Hub.

### Puis-je configurer les champs obligatoires ou facultatifs lors du chargement de ressources vers AEM Assets Content Hub ? {#available-fields-while-uploading-assets-to-content-hub}

Les administrateurs peuvent utiliser l’interface utilisateur de configuration pour définir les champs obligatoires ou facultatifs lors du chargement de ressources vers AEM Assets Content Hub.

### Que dois-je faire si mes ressources chargées ne s’affichent pas automatiquement dans AEM Assets Content Hub ? {#assets-do-not-display-in-content-hub}
