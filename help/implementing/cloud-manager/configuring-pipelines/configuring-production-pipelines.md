...
1. La boîte de dialogue **Ajouter un pipeline de production** s’affiche. Pour identifier votre pipeline, fournissez un **Nom du pipeline** ainsi que les options suivantes. Cliquez sur **Continuer**.
...
1. Dans l’onglet **Code**, sélectionnez le type de code que le pipeline traite.
...
1. Définissez l’**Environnements de déploiement éligibles**.
   ...
   * Si votre pipeline est un pipeline de déploiement, sélectionnez les environnements vers lesquels il déploie.

1. Sous **Code**, définissez les options suivantes :

   * **Référentiel** - cette option définit le référentiel Git à partir duquel le pipeline récupère le code.
   ...
   * **Branche Git** - cette option définit la branche du pipeline sélectionné à partir de laquelle elle récupère le code.
...
Pour activer la création intelligente, voir [Ajouter un pipeline de production](#adding-production-pipeline).

## Ignorer les packages Dispatcher {#skip-dispatcher-packages}

Pour créer des packages Dispatcher dans votre pipeline sans les publier pour créer du stockage, vous pouvez désactiver l’option de publication. Cela permet de réduire le temps d’exécution du pipeline.
