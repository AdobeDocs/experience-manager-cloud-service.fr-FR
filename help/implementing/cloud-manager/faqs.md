...
## Mon build échoue avec une erreur relative à maven-scr-plugin après le passage de Java™ 8 à Java™ 11. Que puis-je faire ? {#build-fails-maven-scr-plugin}

Votre version d’AEM Cloud Manager peut échouer lorsque vous tentez de la faire passer de Java™ 8 à Java11. Si vous rencontrez l’erreur ci-dessous, vous devez supprimer `maven-scr-plugin` et convertir toutes les annotations OSGi en annotations OSGi R6.
...
## Mon build échoue avec une erreur concernant RequireJavaVersion après le passage de Java™ 8 à Java™ 11. Que puis-je faire ? {#build-fails-requirejavaversion}
...
Cette erreur est un problème connu en raison duquel Cloud Manager utilise une autre version de Java™ pour exécuter la commande Maven au lieu de compiler le code. Omettez `requireJavaVersion` de vos configurations `maven-enforcer-plugin`.

## La vérification de la qualité du code a échoué, bloquant le déploiement. Y a-t-il un moyen de contourner cette vérification ? {#deployment-stuck}
...
Cette valeur permet d’installer le déploiement suivant même si la version n’a pas été modifiée. Pour les déploiements de développeurs, aucune version automatique n’est ajoutée ou générée pour la build maven.
...
Dans l’exemple d’erreur précédent, le package `myapp-base.ui.content-*.zip` inclut le contenu situé sous `/conf` et `/var/workflow`. Pour que le déploiement réussisse, les autorisations pour le `sling-distribution-importer` sous ces chemins d’accès sont nécessaires.
