...
Pour découvrir la fonctionnalité, il faut d’abord créer des styles pour un composant. Les sections suivantes, [En tant qu’auteur de contenu](#as-a-content-author) et [En tant qu’auteur de modèles](#as-a-template-author), vous expliquent comment utiliser les fonctionnalités du système de style, en partant du principe que des styles sont déjà configurés pour vos composants.
...
1. Modifiez une page contenant un composant pour lequel un style est déjà configuré.
1. Sélectionnez, comme exemple, un composant dont le style est configuré, tel que le composant **Liste**.

   ![Création de styles](/help/sites-cloud/authoring/assets/style-system-author.png)
...
1. Appuyez ou cliquez sur le bouton **Styles** dans la barre d’outils du composant **Liste** pour ouvrir le menu des styles et changer l’apparence du composant.
   ...
   ![Création de styles au moyen d’une sélection](/help/sites-cloud/authoring/assets/style-system-author-select.png)

   >[!NOTE]
   >
   >Dans cet exemple, les styles de **disposition** (**Bloc** et **Grille**) sont mutuellement exclusifs, tandis que les options d’**affichage** (**Image** ou **Date**) peuvent être combinées. Cela peut être [configuré dans le modèle par l’auteur de modèles](#as-a-template-author).

### En tant qu’auteur de modèles   {#as-a-template-author}

1. Lors de la modification d’une page de contenu pour laquelle vous souhaitez configurer des styles, modifiez le modèle de la page en sélectionnant **Informations sur la page -> Modifier le modèle**.

   ![Modifier le modèle](/help/sites-cloud/authoring/assets/style-system-template.png)

1. Modifiez la stratégie du composant dont vous souhaitez configurer les styles, comme le composant **Liste**, en appuyant ou en cliquant sur le bouton **Stratégie** approprié.
...
1. Dans l’onglet Styles des propriétés, vous pouvez voir comment les styles ont été configurés.
   ...
   ![Onglet Styles de la fenêtre des propriétés](/help/sites-cloud/authoring/assets/style-system-template-styles.png)
...
>[!CAUTION]
>
>Pour pouvoir fonctionner, les classes CSS (ainsi que toute classe JavaScript nécessaire) configurées en tant que propriétés de style d’une stratégie de composants doivent être déployées comme bibliothèques clientes. <!-- The CSS classes (as well as any necessary Javascript) configured as style properties of a component's policy must be deployed as [Client Libraries](/help/sites-developing/clientlibs.md) in order to work.-->
