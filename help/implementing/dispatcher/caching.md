...
Lorsqu’une requête HEAD est reçue sur le réseau CDN Adobe pour une ressource qui n’est **pas** mise en cache, la requête est transformée et reçue par le Dispatcher et/ou l’instance AEM en tant que requête GET. Si la réponse peut être mise en cache, les requêtes HEAD suivantes seront diffusées à partir du réseau CDN. Si la réponse ne peut pas être mise en cache, les requêtes HEAD suivantes seront transmises au Dispatcher et/ou à l’instance AEM pendant une période qui dépend du TTL `Cache-Control`.

### Paramètres de campagne marketing {#marketing-parameters}

Les URL de site web incluent souvent des paramètres de campagne marketing qui permettent de suivre la réussite d’une campagne. Pour utiliser efficacement le cache du Dispatcher, il est recommandé de définir la propriété `ignoreUrlParams` de la configuration du Dispatcher comme [décrit ici](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=fr#ignoring-url-parameters).
