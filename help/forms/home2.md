...
**Parcours De Workflow AEM Forms :**
    ...
    CRÉER → GOUVERNER → PUBLIER → CAPTURER → PROCESSUS → INTÉGRER → SUIVRE → ARCHIVER → AMÉLIORER
    ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↑ ↓
    Conception Réviser Déployer Collecter Poignée Connecter Moniteur Optimiser le magasin
    ←←←←←←←←←←←←←←← ←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←
