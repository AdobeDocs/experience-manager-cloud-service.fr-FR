---
title: Comment utiliser CAPTCHA dans le Forms adaptatif ?
description: Découvrez comment configurer ou le service Google reCAPTCHA pour un formulaire adaptatif.
...
# Utilisation de reCAPTCHA dans le Forms adaptatif {#using-reCAPTCHA-in-adaptive-forms}
...
1. Configurez le service cloud pour reCAPTCHA v2.
   ...
   1. Sélectionnez **[!UICONTROL reCAPTCHA]**. La page de configuration s’ouvre. Sélectionnez le conteneur de configurations que vous avez créé et sélectionnez **[!UICONTROL Créer]**.
   1. Sélectionnez la version comme [!DNL reCAPTCHA v2] , spécifiez le nom, la clé de site et la clé secrète pour le service reCAPTCHA (obtenue à l’étape 1), puis sélectionnez **[!UICONTROL Créer]** pour créer la configuration du service cloud.
...
1. À partir du navigateur du composant, faites glisser et déposez le composant **Captcha** sur le formulaire adaptatif.
   ...
   >* reCaptcha est sensible au facteur temps et expire dans environ deux minutes. Par conséquent, il est recommandé de placer le composant Captcha juste avant le bouton Soumettre dans le formulaire adaptatif.
...
1. Sélectionnez une configuration dans le menu déroulant Paramètres pour **reCAPTCHA Enterprise** ou **reCAPTCHA v2**
   ...
   1. Vous pouvez sélectionner une **[!UICONTROL Référence de liaison]**, dans **[!UICONTROL Référence de liaison]** les données envoyées sont des données liées, sinon elles sont non liées. Vous trouverez ci-dessous des exemples XML de données non liées et de données liées (avec une référence de liaison telle que SSN), respectivement, lorsqu’un formulaire est soumis.
