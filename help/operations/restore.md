...
* **Point d’entrée (PIT) :** Ce type effectue une restauration à partir de sauvegardes système continues des dernières 24 heures à partir de l’heure actuelle.
...
* Si un package pour le chemin supprimé est disponible, réinstallez-le à l’aide du [gestionnaire de modules.](/help/implementing/developing/tools/package-manager.md)
...
* Si le chemin d’accès supprimé était un dossier de ressources et que les fichiers d’origine sont disponibles, téléchargez-les à nouveau via [la console Assets.](/help/assets/add-assets.md)
* Si le contenu de suppression était des ressources, pensez à [restaurer les versions précédentes des ressources.](/help/assets/manage-digital-assets.md)
...
1. Fournissez l’autorisation **Content Restore** sur l’environnement requis ou tous les environnements du programme, en fonction de votre cas d’utilisation.
...
1. Sur la page **Aperçu du programme**, dans la carte **Environnements**, cliquez sur le bouton représentant des points de suspension en regard de l’environnement pour lequel vous souhaitez lancer une restauration, puis sélectionnez **Restaurer le contenu**.
...
1. Sur l’onglet **Restaurer le contenu** de la page des détails de l’environnement, sélectionnez tout d’abord la période de restauration dans la liste déroulante **Temps de restauration** .

   1. Si vous sélectionnez **Dernières 24 heures**, le champ **Heure** voisin vous permet de spécifier l’heure exacte au cours des dernières 24 heures à restaurer.
...
1. Recherchez la sauvegarde à restaurer à l’aide de l’icône d’information pour afficher les informations concernant la version du code et la version d’AEM inclus dans cette sauvegarde et prendre en compte les implications d’une restauration lors de [choix de la sauvegarde.](#choosing-the-right-backup)
...
1. Cliquez sur l’icône **Restaurer** située à droite de la ligne représentant la sauvegarde à restaurer pour lancer le processus de restauration.
...
Le processus de sauvegarde est lancé et vous pouvez afficher son état dans la liste **[Restaurer l’activité](#restore-activity)**. Le temps nécessaire à l’achèvement d’une opération de restauration dépend de la taille et du profil du contenu restauré.
...
La liste **Restaurer l’activité** indique l’état des dix demandes de restauration les plus récentes, y compris toutes les opérations de restauration actives.
