...
* Noms des propriétés de noeud qui contiennent le contenu à traduire
   * Cette propriété peut être spécifique à un type de ressource en particulier ou à tous les types de ressource.

Par exemple, vous pouvez créer une règle qui traduit le contenu que les auteurs ajoutent à tous les composants de texte de vos pages. La règle peut identifier le noeud `/content` et la propriété `text` du composant `core/wcm/components/text/v2/text`.
...
L&#39;exemple de règle suivant fait traduire le contenu de toutes les propriétés `text` pour toutes les pages situées sous le noeud `/content`. La règle est efficace pour tout composant qui stocke du contenu dans une propriété `text`, telle que le composant de texte.
...
L’exemple suivant extrait des images du composant d’image :
...
1. **Contexte** Ajouté, qui vous permet d’ajouter un chemin d’accès.
...
1. Ensuite, vous devez sélectionner votre contexte, puis cliquer sur **Modifier**. Cette action ouvre l’éditeur de règles de traduction.
...
**`isDeep`**  est applicable sur les filtres de noeud et est true par défaut. Il vérifie si le nœud (ou ses ancêtres) contient cette propriété avec la valeur de propriété spécifiée dans le filtre. S’il a la valeur false, il vérifie uniquement le nœud actuel.
...
**`inherit`** est applicable aux propriétés. Par défaut, chaque propriété est héritée, mais si vous souhaitez que certaines propriétés ne soient pas héritées par l’enfant, vous pouvez marquer cette propriété comme étant fausse afin qu’elle ne soit appliquée qu’à ce noeud spécifique.

Dans l’interface utilisateur, vous pouvez cocher/décocher **Hériter** dans l’onglet **Propriétés**.

### translate {#translate}

**`translate`** est simplement utilisée pour spécifier si une propriété doit être traduite ou non.

Dans l’interface utilisateur, vous pouvez vérifier/décocher **Traduire** dans l’onglet **Propriétés**.

### updateDestinationLanguage {#updatedestinationlanguage}

**`updateDestinationLanguage`** est utilisée pour les propriétés qui n’ont pas de texte mais des codes de langue, par exemple  `jcr:language`. L’utilisateur ne traduit pas le texte, mais le paramètre régional de langue de la source vers la cible. Ces propriétés ne sont pas envoyées en traduction.

Dans l’interface utilisateur, vous pouvez cocher/décocher **Traduire** dans l’onglet **Propriétés** pour modifier cette valeur, mais pour les propriétés spécifiques dont les codes de langue sont des valeurs.
