...
* FORMS-15113 - Modification du nom de la clé en sid à partir de vistorId pour le suivi de la télémétrie opérationnelle.
...
* FORMS-14275 - Remplacement de la page de remerciement et du message de remerciement dans le conteneur incorporé.
...
* CQ-4357059 - La tâche de traduction met des heures à se terminer, ce qui entraîne une erreur 503 de temporisation dans AEMaaCS.
...
* FORMS-14327 - API du service Forms : extraction de données:- fournit un code de réponse 500 lorsqu’un pdf non interactif est fourni en entrée.
...
* FORMS-15818 - L’entrée de descripteur de composant « OSGI-INF/com.adobe.aemfd.docmanager.impl.*.xml » est introuvable dans les instructions des journaux du serveur. Ce sont des instructions de journal inoffensives.
...
Nous mettons à jour en ce moment `com.day.cq.wcm.api`. Dans la version actuelle, nous avons déclaré certaines de ses méthodes et classes comme `@Deprecated`. Celles-ci seront supprimées des prochaines versions. Envisagez donc de passer à leurs alternatives qui sont suggérées si vous utilisez l’une d’elles.
...
* ASSETS-31445 - Fonctionnalités de création de modèles Dynamic Media initiales.
* ASSETS-40399 - Mise à jour des paramètres de file d’attente de transcription automatique DM.
* ASSETS-40873 - Autoriser la définition du nombre maximal de lignes d’exportation de métadonnées via la configuration OSGI.
...
* ASSETS-31882 - L’accès au fichier manifeste de diffusion en continu dans l’instance de création est interdit.
* ASSETS-39598 - L’importation en bloc ne peut pas supprimer les ressources dont le nom contient des caractères spéciaux du serveur principal S3.
* CNTBF-209 - Améliorations de l’annulation des tâches de retour.
* SCRNS-3762 - Améliorez l’enregistreur de lecteur dans le canal de séquence pour placer les journaux sur la console lors de la prévisualisation du canal sur le navigateur.
* SCRNS-4455 - Pas de bouton « Gérer la publication » et « Publication rapide » pour les utilisateurs NON-ADMINISTRATEURS dans le fournisseur de contenu pour les canaux.
