---
title: Personnaliser le gestionnaire de contenu
description: Utilisez des fonctions pour personnaliser le gestionnaire d'accès dans votre application.
role: Admin, User
...
# Personnalisations du gestionnaire d’accès {#content-advisor-customization}

Le gestionnaire d’accès vous permet de personnaliser divers composants en fonction des préférences, des exigences ou des besoins fonctionnels. Vous pouvez personnaliser les composants suivants [gestionnaire d’accès micro front-end](#overview-asset-selector.md) :
...
Vous pouvez personnaliser l’utilisation d’une ressource arrivée à expiration afin de la rendre sélectionnable ou non. Vous pouvez choisir d’autoriser ou non le glisser-déposer d’une ressource arrivée à expiration sur la zone de travail du gestionnaire d’accès. Pour ce faire, utilisez les paramètres suivants pour rendre une ressource non sélectionnable sur la zone de travail :
...
## Charger dans le gestionnaire d’accès {#upload-in-content-advisor}

Vous pouvez charger des fichiers ou des dossiers vers le gestionnaire de contenu à partir de votre système de fichiers local. Pour charger des fichiers à l’aide du système de fichiers local, vous devez généralement utiliser une fonctionnalité de chargement fournie par une micro application front-end de la fonction de conseil en contenu. Les `upload` divers fragments de code requis pour appeler le chargement dans le gestionnaire d&#39;accès sont les suivants :
...
*D’autres propriétés incluent `metadataSchema`, `onMetadataFormChange`, `targetUploadPath`, `hideUploadButton`, `onUploadStart`, `importSettings` `onUploadComplete`, `onFilesChange` et`uploadingPlaceholder`*. Voir [Propriétés du gestionnaire d’accès](/help/assets/content-advisor-properties.md) pour plus d’informations.
