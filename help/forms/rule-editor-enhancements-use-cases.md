...
| [Combiné lorsque des conditions avec le composant Pièce jointe](#combined-when-conditions-with-the-file-attachment-component) | Créez une règle Lorsque pour le composant Pièce jointe à l’aide de la logique Ajouter une condition et AND ou OR afin que la pièce jointe soit évaluée avec d’autres validations. | - Les actions s’exécutent uniquement lorsque l’état de la pièce jointe et d’autres vérifications sont évalués comme prévu <br> - Moins de règles chaînées pour les scénarios de chargement <br> - Création plus claire pour les formulaires qui nécessitent à la fois des fichiers et des entrées validées |
...
### grammaire simplifiée pour les événements prêts à l’emploi et personnalisés {#simplified-grammar-for-ootb-and-custom-events}

L’éditeur de règles amélioré comprend une **grammaire simplifiée** pour les règles basées sur un événement qui utilisent **Distribuer l’événement** et **Activer l’événement déclencheur**. Auparavant, cette grammaire s’appliquait uniquement aux événements **personnalisés** ; les événements prêts à l’emploi n’étaient pas pris en charge, ce qui nécessitait souvent des règles **Lorsque** pour les déclencheurs prêts à l’emploi et des règles **Sur l’événement déclencheur** pour les événements personnalisés. Les événements prêts à l’emploi sont désormais pris en charge avec la même grammaire simplifiée, ce qui permet d’obtenir un modèle de création cohérent sans passer de **Quand** à **Événement de déclencheur activé** selon que le déclencheur est prêt à l’emploi ou personnalisé.
...
## Conditions Lorsque combinées avec le composant Pièce jointe {#combined-when-conditions-with-the-file-attachment-component}

L’éditeur de règles amélioré prend en charge les règles **Lorsque** qui combinent le composant **Pièce jointe** avec d’autres conditions à l’aide de la logique **AND** ou **OR**. **Ajouter une condition** dans la clause **Lorsque** peut inclure l’état de la pièce jointe avec des vérifications sur d’autres champs ou la validation du panneau, de sorte qu’une action ne s’exécute que lorsque chaque condition sélectionnée est remplie.

**Scénario** : Un formulaire d&#39;enregistrement d&#39;animal de compagnie collecte **Identifiant de l&#39;animal**, **Nom de l&#39;animal** et **Catégorie de l&#39;animal**, et comprend une pièce jointe **Ajouter une photo**. Le formulaire exécute une action, par exemple effacer ou actualiser **Ajouter une photo**, lorsque la pièce jointe change **et** que les conditions configurées dans les autres champs (leurs valeurs) sont remplies.

**Implémentation à l’aide des conditions Lorsque avec le composant Pièce jointe dans l’éditeur de règles**

Une règle est configurée sur l’objet cible (par exemple, **Ajouter une photo**). La section **Lorsque** utilise **Ajouter une condition** pour combiner le déclencheur de pièce jointe avec des conditions sur un ou plusieurs autres champs, de sorte que l’action dépend à la fois de la pièce jointe et de ces valeurs de champ.

La capture d’écran ci-dessous affiche la condition **Lorsque** avec plusieurs conditions et les options **Ajouter une condition** :

![Lorsque la règle comporte plusieurs conditions et Ajouter une condition](/help/forms/assets/rule-editor-when-file-attachment-conditions.png)

Lorsque la clause **When** est évaluée comme vraie pour la logique **AND** ou **OR** configurée, la règle exécute l’action configurée.

>[!VIDEO](https://video.tv.adobe.com/v/3483735/file-attachment/?quality=12&learn=on)

Lorsque **Pet ID** contient `101`, la pièce jointe **Ajouter une photo** disparaît ; de même, lorsque **Pet Name** contient `a`, la pièce jointe disparaît.
...
| Paramètre UTM | Contenu UTM <br>UTM Medium<br>UTM Campaign<br>UTM Term<br>UTM | Paramètres de requête spéciaux utilisés pour le tracking des campagnes | `?utm_source=google&utm_medium=email` |
