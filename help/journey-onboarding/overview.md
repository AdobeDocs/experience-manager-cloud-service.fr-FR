...
Félicitations pour avoir choisi AEM as a Cloud Service. Ce document constitue le point de départ d’un parcours guidé tout au long du processus d’intégration. Que vous déployiez une nouvelle application ou migriez une application existante, ce parcours d’intégration configure vos équipes. Il garantit qu’ils ont accès à AEM as a Cloud Service.
...
* Accorder à AEM as a Cloud Service l’accès de l’utilisateur nécessaire.
...
Le parcours d’intégration est écrit spécifiquement pour l’ **administrateur système** des clients qui découvrent AEM as a Cloud Service et AEM en général. L’administrateur système est la personne qui a Adobe les premiers contacts après la signature de votre contrat AEM as a Cloud Service. Il s’agit généralement de la première personne à accéder à vos ressources et à les configurer sur AEM as a Cloud Service. Si vous lisez cette rubrique, il est probable que vous soyez l’administrateur ou l’administratrice système.

L’administrateur système gère tous les aspects des utilisateurs AEMaaCS de son entreprise, de l’accès aux autorisations. Cependant, l’administrateur système doit interagir avec d’autres personnes en cours de route.

| Personne | Description | Rôle dans le Parcours |
|---|---|---|
| Administrateur système | La cible de ce parcours fournit la configuration initiale des ressources cloud et l’affectation des utilisateurs aux rôles appropriés en fonction de leurs responsabilités professionnelles. | Gère tous les aspects liés aux utilisateurs, depuis l’accès jusqu’aux autorisations |
| Créateur de contenu | Crée et révise le contenu dans AEM | Une fois les autorisations accordées par l’administrateur système, les auteurs peuvent commencer leur propre parcours dans la création de contenu. |
| Développeur ou développeuse | Développe des applications AEM qui consomment du contenu provenant de différentes sources. | Une fois les autorisations accordées par l’administrateur système, les développeurs peuvent démarrer leur propre parcours dans le développement de solutions. |
...
>[!TIP]
>
>Si vous découvrez AEM as a Cloud Service et que vous connaissez AEM et que vous migrez depuis Managed Services On-Premise ou Adobe, consultez le [Parcours de migration AEM as a Cloud Service](/help/journey-migration/getting-started.md).
...
| 10 | [Tâches utilisateur d’AEM](aem-users.md) | Facultatif - Découvrez comment, en tant qu’auteur ou autrice AEM, vous pouvez accéder à l’instance AEM as a Cloud Service et vous familiariser avec la création de contenu pour AEM as a Cloud Service. | Utilisateurs AEM |
...
[Un parcours de documentation](/help/journey-documentation/documentation-journeys.md) associe de nombreuses rubriques et fonctionnalités différentes et complexes. Il fournit un récit qui aide un lecteur ou une lectrice nouveau/nouvelle sur AEM à comprendre et résoudre un problème d’entreprise du début à la fin, tout en assumant un minimum de sujets antérieurs ou de connaissances AEM.

Les Parcours de documentation sont conçus autour des principes des bonnes pratiques. Elles sont informées à l’aide des dernières recherches d’Adobe, de l’expérience de mise en oeuvre éprouvée des consultants Adobe et des commentaires des projets clients.

Si vous souhaitez connaître les recommandations d’Adobe concernant l’intégration de votre équipe à votre nouvelle application AEM as a Cloud Service, commencez ici.
...
* [Intégration dans AEM as a Cloud Service](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/migration/moving-to-aem-as-a-cloud-service/onboarding) : cette courte vidéo propose une vue d’ensemble du processus d’intégration du service cloud pour AEM.
