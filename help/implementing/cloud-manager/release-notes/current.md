...
La date de publication de la version 2025.8.0 de Cloud Manager dans AEM as a Cloud Service est le vendredi 7 août 2025.
...
* La licence **Edge Delivery Services peut être incluse dans un programme HIPAA en libre-service**
  ...
  Les entreprises ayant des besoins en matière de soins de santé ou de données sensibles peuvent désormais utiliser Edge Delivery Services en libre-service, ce qui permet aux entreprises conformes à la loi HIPAA de respecter des normes réglementaires strictes. <!-- CMGR-70016 -->

* **BYOG est désormais disponible pour Edge Delivery Services**

  Cloud Manager vous permet désormais de configurer des référentiels Git externes et d’activer des workflows de gestion de code flexibles. <!--(CMGR‑69010, CMGR‑70988) --> Il vous permet également d’extraire du code d’une branche sélectionnée directement dans l’interface utilisateur de Cloud Manager, ce qui réduit les tâches de référentiel manuelles. Voir [Configuration d’un site Edge Delivery pour utiliser un référentiel Git externe](/help/implementing/cloud-manager/edge-delivery/config-edge-delivery-site-with-byog.md) <!-- (CMGR‑68085)(CMGR-69015) --> <!-- KT: https://wiki.corp.adobe.com/display/DMSArchitecture/%5B2025%5D+Cloud+Manager+-+Bring+Your+Own+Git+with+EDS -->

* **Approvisionnement automatisé pour le nouveau module complémentaire Forms**

  Les clients Sites uniquement ont souvent besoin d’un moyen léger et économique de créer des formulaires marketing. Le nouveau module complémentaire AEM Forms Sites répond à ce besoin en ajoutant des fonctionnalités Forms limitées à un programme Sites. Cela permet également de créer un chemin de mise à niveau clair vers l’offre AEM Forms complète, si vous le souhaitez. <!-- (CMGR-64301) --> <!-- KT: CMGR Provisioning Support for AEM Forms Sites Add-On SKU https://wiki.corp.adobe.com/pages/viewpage.action?pageId=3578379797 -->

  Le module complémentaire :
   * Se connecte à un programme Sites et se déploie en parallèle, sans programme ni droit Forms distinct.
   * Cible les cas d’utilisation de formulaires marketing simples.
   * S’affiche dans la liste **Solutions et modules complémentaires** lors de la création du programme de production ou de la modification du programme de production, uniquement lorsque l’organisation IMS détient des licences de modules complémentaires Forms disponibles.
     ...
     Pour plus d’informations, voir [Créer un programme de production](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/creating-production-programs.md).

## Programmes Beta {#private-beta-program}

Participez aux programmes bêta de Cloud Manager pour obtenir un accès exclusif aux fonctionnalités à venir avant leur publication générale.
...
* Vous pouvez désormais configurer un environnement de test spécialisé sur un pipeline hors production grâce à un workflow plus simple et plus intuitif. La configuration rationalisée accélère l’achèvement et réduit les erreurs de configuration.
* La **copie de contenu** est désormais prise en charge dans les environnements de test spécialisés. Vous pouvez désormais exécuter **Copier le contenu** en toute sécurité dans des environnements de test isolés qui reflètent la production. <!-- (CMGR‑68900) -->
...
| *Les paramètres existants tels que les listes `IP Allow` continuent-ils à fonctionner ?* | Oui, les listes de `IP Allow` existantes continuent de fonctionner comme d’habitude. Cependant, si le référentiel Git externe est protégé par un pare-feu, les [adresses IP Adobe nécessaires doivent être ajoutées à la liste autorisée](/help/implementing/cloud-manager/ip-allow-lists/introduction.md). |
...
* Cloud Manager affiche **Ajouter un pipeline Edge Delivery** une fois qu’un programme contient au moins un site Edge Delivery Services et un domaine mappé. Dans le cas contraire, l’option est désactivée et une info-bulle explique les exigences manquantes. <!-- CMGR‑69680 -->
...
* Le panneau **Filtres** ajoute une section **Type de diffusion** qui comprend les cases à cocher **Diffusion Edge** et **Publier la diffusion**. <!-- (CMGR-69682) -->

  ![Panneau de filtrage présentant le nouveau type de diffusion de la diffusion Edge et la diffusion Publication](/help/implementing/cloud-manager/release-notes/assets/filter-delivery-type.png)
...
* Les pipelines ne fournissent désormais des variables qu’à la configuration de domaine Edge Delivery Services principale, en ignorant toute configuration supprimée lors de la recréation du pipeline. <!-- (CMGR‑70039) -->
* L’exécution du pipeline démarre désormais de manière fiable. Correction d’un problème où certains pipelines ne démarraient pas en raison d’erreurs de gestion des ressources internes. <!-- (CMGR‑58167) -->
* La copie de contenu valide les autorisations Cloud Manager et bloque les démarrages par les utilisateurs ne disposant pas des droits de responsable de déploiement ou d’administrateur. <!-- (CMGR‑62097) -->
