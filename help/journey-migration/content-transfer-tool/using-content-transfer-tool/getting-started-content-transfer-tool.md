...
>[!NOTE]
>
>Une erreur de connexion peut également se produire si un ensemble de migration a été supprimé de Cloud Acceleration Manager.
...
>abstract="Découvrez comment créer et gérer un ensemble de migration et comment copier la clé d’extraction."
>additional-url="https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/migration/content-transfer-tool.html?lang=fr#migration" text="Tutoriel – Utilisation de l’outil de transfert de contenu"
...
1. Créez un jeu de migration en cliquant sur **Créer un jeu de migration**.
   ...
   La boîte de dialogue suivante s’affiche. Notez qu’un ensemble de migration expire après une longue période d’inactivité. Une fois les avertissements affichés sur la carte du projet et les lignes du tableau des tâches de migration pendnat un certain temps, le jeu de migration expire et ses données ne sont plus disponibles. Consultez [Expiration de l’ensemble de migration](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/overview-content-transfer-tool.md#migration-set-expiry) pour plus d’informations.
...
1. Votre liste de migration doit maintenant apparaître dans la vue Liste. Sélectionnez le symbole des trois petits points (**...**) pour ouvrir la liste déroulante, puis sélectionnez **Copier la clé d’extraction**. Vous avez besoin de cette clé pendant la phase d’extraction. Copiez cette clé d’extraction.
   ...
   >[!NOTE]
   >
   >La clé d’extraction permet à votre environnement AEM source de se connecter en toute sécurité à l’ensemble de migration. Traitez cette clé avec le même soin que vous le feriez avec un mot de passe et ne la partagez jamais sur un support non sécurisé comme un e-mail.

   ![image](/help/journey-migration/content-transfer-tool/assets-ctt/cttcam4.png)

### Renseignement de l’ensemble de migration {#populating-the-migration-set}

>[!CONTEXTUALHELP]
>id="aemcloud_ctt_populate_migrationset"
>title="Renseigner l’ensemble de migration"
>abstract="Lorsque vous avez créé un ensemble de migration, celui-ci doit être renseigné avec le contenu de l’instance source qui doit être déplacée vers l’environnement AEM as a Cloud Service. Pour ce faire, l’outil de transfert de contenu doit être installé sur l’instance source."
...
1. Collez la clé d’extraction qui a été copiée à partir de l’instance CAM plus tôt dans le champ d’entrée Clé d’extraction du formulaire **Créer un ensemble de migration**. Une fois que vous avez effectué cette opération, les champs Nom de l’ensemble de migration et Nom du projet de Cloud Acceleration Manager (CAM) sont automatiquement renseignés. Ils doivent correspondre au nom de l’ensemble de migration dans CAM et au nom du projet CAM que vous avez créé. Vous pouvez maintenant ajouter des chemins de contenu. Après avoir ajouté des chemins de contenu, enregistrez l’ensemble de migration. Vous pouvez exécuter l’extraction en incluant ou en excluant les versions.

   >[!NOTE]
   >
   >Assurez-vous que la clé d’extraction est valide et n’est pas proche de son expiration. Vous pouvez obtenir ces informations dans la boîte de dialogue **Créer un ensemble de migration** après avoir collé la clé d’extraction. Si vous obtenez une erreur de connexion, reportez-vous à [Connectivité de l’environnement source](#source-environment-connectivity) pour plus d’informations.

   ![image](/help/journey-migration/content-transfer-tool/assets-ctt/createMigrationSet.png)

1. Sélectionnez ensuite les paramètres suivants pour créer un ensemble de migration :
...
1. Cliquez sur **Enregistrer** après avoir rempli tous les champs de l’écran **Créer un ensemble de migration**.
...
### Vérification de la taille de l’ensemble de migration {#migration-set-size}

Après la création d’un ensemble de migration, il est vivement recommandé d’exécuter une vérification de taille sur l’ensemble de migration avant de lancer un processus d’extraction.
En effectuant une vérification de taille sur l’ensemble de migration, vous pouvez :

* déterminer s’il y a suffisamment d’espace disque dans le sous-référentiel `crx-quickstart` pour terminer l’extraction ;
* déterminer si la taille de l’ensemble de migration est conforme aux restrictions concernant les produits pris en charge et éviter les échecs d’ingestion de contenu.
...
1. Cliquez sur **Check Size** (Vérifier la taille) pour lancer le processus. Vous revenez alors à la vue Liste de l’ensemble de migration et un message vous indique que **Vérifier la taille** est en cours d’exécution.
...
1. Si les résultats du processus **Vérifier la taille** indiquent qu’il n’y a pas suffisamment d’espace disque et/ou que l’ensemble de migration dépasse les limites du produit, le statut **AVERTISSEMENT** s’affiche.
