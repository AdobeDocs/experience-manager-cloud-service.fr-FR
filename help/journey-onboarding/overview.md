...
Félicitations pour avoir choisi AEM as a Cloud Service. Ce document constitue le point de départ d’un parcours guidé tout au long du processus d’intégration. Que vous déployiez une nouvelle application ou migriez une application existante, ce parcours d’intégration garantit que vos équipes sont prêtes et ont accès à AEM as a Cloud Service.

## Présentation  {#introduction}

L’intégration est le processus au cours duquel un administrateur système désigné configure AEM as a Cloud Service pour votre organisation. Il inclut l’approvisionnement initial des ressources cloud et l’affectation des utilisateurs à des rôles en fonction de leurs responsabilités professionnelles. Par conséquent, chaque membre peut se connecter et accéder à ses ressources AEM as a Cloud Service.
...
* Votre équipe sera prête et pourra accéder aux ressources cloud.
* Les créateurs AEM auront accès à AEM as a Cloud Service et pourront commencer à créer du contenu.
...
* **Le contrat** - Vous devez maîtriser votre contrat Adobe, car il définit les aspects du processus d’intégration.
* **Admin Console** - C’est là que les utilisateurs sont gérés et que les rôles sont attribués.
...
* Ayez accordé aux utilisateurs l’accès nécessaire à AEMaaCS
* Ayez configuré les premières ressources cloud pour votre projet ;
...
Le parcours d’intégration est écrit spécifiquement pour l’**administrateur système** des nouveaux clients d’AEM as a Cloud Service et d’AEM en général. L’administrateur système est la personne qui est contactée la première par Adobe une fois que votre contrat AEM as a Cloud Service est signé, et est généralement la première personne à accéder à vos ressources AEM as a Cloud Service et à les configurer. Si vous lisez ceci, c’est que vous êtes probablement l’administrateur système.
...
| 8 | [Attribuer des profils de produit AEM](assign-profiles-aem.md) | Découvrez comment l’administrateur système attribue les membres de votre équipe à des profils de produit AEM as a Cloud Service. | Administrateur système |
...
| 10 | [Tâches utilisateur d’AEM](aem-users.md) | Facultatif - Découvrez comment, en tant que créateur AEM, vous pouvez accéder à l’instance AEM as a Cloud Service et vous familiariser avec la création de contenu pour AEM as a Cloud Service. | Utilisateurs AEM |
