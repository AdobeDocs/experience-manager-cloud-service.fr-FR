...
1. Choisissez le framework de test à utiliser.
   ...
   * Pour Cypress (par défaut), utilisez l’exemple de code du référentiel d’exemples de test [](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-cypress) ou utilisez l’exemple de code généré automatiquement dans le dossier `ui.tests` de votre référentiel Cloud Manager.

   * Pour Playwright, utilisez l’exemple de code du [Référentiel d’exemples de test ](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-playwright).

   * Pour Webdriver.IO, utilisez l’exemple de code du [Référentiel d’exemples de test ](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-wdio).

   * Pour Selenium WebDriver, utilisez l’exemple de code du [Référentiel d’exemples de test ](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-selenium-webdriver).
...
>[!TIP]
>
>L’[archétype de projet ](https://github.com/adobe/aem-project-archetype) peut générer pour vous un projet de tests de l’interface utilisateur, conforme à la description suivante, si vous n’avez pas d’exigences spécifiques pour le langage de programmation.
...
Il est recommandé de configurer le [plug-in Maven Assembly](https://maven.apache.org/plugins/maven-assembly-plugin/) pour créer l’archive de contexte de création Docker et lui affecter le classificateur approprié.
...
Cette exécution indique au plug-in Maven Assembly de créer une archive basée sur les instructions contenues dans `assembly-ui-test-docker-context.xml`, appelée descripteur d’assemblage **** dans la terminologie du plug-in. Le descripteur d’assemblage répertorie tous les fichiers qui doivent faire partie de l’archive.
...
Le descripteur d’assembly exclut également les fichiers générés lors de l’exécution locale des tests de l’interface utilisateur. Ce processus garantit une archive plus petite et des versions plus rapides.
...
Le build produit zéro ou une archive. S’il ne produit aucune archive, l’étape de test est effectuée par défaut. Si le build produit plusieurs archives, celle qui est sélectionnée est non déterministe.
...
>
>Le fichier contient une ligne qui indique *NE PAS MODIFIER*. Il s’agit d’un avertissement hérité des anciens modèles/exemples qui ne vous empêche *pas* d’effectuer les modifications de souscription requises pour les tests de l’interface utilisateur de Cloud Manager. Vous pouvez ignorer le conseil ; vous pouvez modifier les `assembly-ui-test-docker-context.xml` et les `pom.xml` dans *votre projet* lorsque vous suivez les étapes d’accord préalable (par exemple, pour inclure des `testing.properties`).

Si vous utilisez les exemples fournis par Adobe :

* Pour ajouter la configuration requise pour le dossier `ui.tests` basé sur JavaScript généré à partir de l’[archétype de projet AEM](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/ui.tests), exécutez la commande suivante.
...
L’image Docker doit générer des rapports de test au format XML `JUnit` et les enregistrer dans le chemin spécifié par la variable d’environnement `REPORTS_PATH`. Le format XML `JUnit` est un format très répandu pour le compte rendu des résultats de tests. Si l’image Docker utilise Java et Maven, les modules de test standard tels que [Maven Surefire Plugin](https://maven.apache.org/surefire/maven-surefire-plugin/) et [Maven Failsafe Plugin](https://maven.apache.org/surefire/maven-failsafe-plugin/) peuvent générer automatiquement de tels rapports.
...
>
>Utilisez des assertions au lieu de consigner une erreur dans STDERR ou de renvoyer un code de sortie non nul, ou votre pipeline de déploiement se poursuit normalement.
>
...
>[!NOTE]
>
>Pour obtenir le même comportement lors de l’exécution de tests fonctionnels à partir de votre ordinateur local, créez un utilisateur ou une utilisatrice avec des autorisations de type administrateur.

* Les éléments suivants limitent l’infrastructure en conteneur qui est prévue pour les tests fonctionnels :
...
### Pour vous assurer que Selenium est prêt, attendez-le. {#waiting-for-selenium}
...
Une fois que le point d’entrée de statut Selenium donne une réponse positive, les tests peuvent commencer.
...
L’image Docker génère une sortie de test supplémentaire (par exemple, des captures d’écran ou des vidéos) et les enregistre dans le chemin spécifié par la variable d’environnement `REPORTS_PATH`. Tout fichier situé sous le `REPORTS_PATH` est inclus dans l’archive des résultats du test.
...
1. Configurez une connexion proxy HTTP pour exécuter des tests d’interface utilisateur en exportant la variable d’environnement `HTTP_PROXY` créée à l’aide des valeurs suivantes :
   ...
   * Port du proxy fourni par la variable `PROXY_HTTPS_PORT` ou `PROXY_HTTP_PORT` (la variable avec une valeur non vide est utilisée).
2. Définissez le certificat d’autorité de certification utilisé lors de la connexion au proxy HTTP. La variable `PROXY_CA_PATH` indique son emplacement.
   * Exportez la variable d’environnement `NODE_EXTRA_CA_CERTS`.
3. Patientez jusqu’à ce que le proxy HTTP soit prêt.
   * Pour vérifier le niveau de préparation, les variables d’environnement `PROXY_HOST`, `PROXY_OBSERVABILITY_PORT`, `PROXY_RETRY_ATTEMPTS` et `PROXY_RETRY_DELAY` peuvent être utilisées.
...
Installer cURL et `libnss3-tools`, qui fournissent des `certutil.`
...
Pour utiliser un proxy dans le cas où la variable d’environnement `HTTP_PROXY` est définie, modifiez la configuration du auteur de lecture (par exemple dans `playwright.config.js`).
...
>
>Pour plus d’informations, consultez le [référentiel d’exemples de test ](https://github.com/adobe/aem-test-samples/blob/aem-cloud/ui-cypress/test-module/README.md).
...
1. Pour démarrer les tests à l’aide de Maven, exécutez la commande suivante.
...
>
>Pour plus d’informations, consultez le [référentiel d’exemples de test ](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-wdio).

### Exemple de test Playwright {#playwright-sample}

1. Ouvrez un conteneur et accédez au dossier `ui.tests` dans votre référentiel

1. Pour créer l’image docker à l’aide de Maven, exécutez la commande suivante :
...
>Les fichiers journaux sont stockés dans le dossier `target/` de votre référentiel.
>
>Pour plus d’informations, consultez le [référentiel d’exemples de test ](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-playwright).
...
1. Pour démarrer les tests à l’aide de Maven, exécutez les commandes suivantes :
...
>Les fichiers journaux sont stockés dans le dossier `target/reports` de votre référentiel.
>
>Pour plus d’informations, consultez le [référentiel d’exemples de test ](https://github.com/adobe/aem-test-samples/blob/aem-cloud/ui-selenium-webdriver/README.md).
