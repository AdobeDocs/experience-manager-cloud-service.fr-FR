...
Le tableau de bord des licences Cloud Manager permet d’accéder facilement aux droits sur les solutions disponibles pour l’ensemble de vos programmes, y compris le nombre de licences utilisées et le nombre de licences disponibles. Les mesures de consommation des demandes de contenu présentent des tendances par mois pour la solution Sites.

## Accès au tableau de bord des licences {#using-dashboard}

>[!NOTE]
>
>Pour afficher le tableau de bord des licences, une personne possédant le rôle **Propriétaire de l’entreprise** doit être connectée.
...
* **Autres droits** - Quels environnements de sandbox et de développement et autres droits peuvent être consommés au sein de votre client ?
...
* La colonne **Configuré le** répertorie les programmes auxquels sont appliqués les droits de la solution.
   ...
   * Sélectionnez l’entrée `+x` pour afficher un pop-up avec les détails de tous les programmes.
...
Le côté droit de la fenêtre présente un graphique en aires montrant l’utilisation répartie par programme au fil du temps pour l’année de contrat sélectionnée. La sélection du point de données affiche un pop-up avec des détails par programme pour le point dans le temps sélectionné.
...
Les demandes de contenu excluent les demandes ou activités initiées par ou pour le compte d’Adobe dans le seul but de fournir des produits et des services. Le trafic des agents utilisateurs identifiés par Adobe provenant des robots, des robots d&#39;exploration et des araignées liés aux moteurs de recherche courants et aux services de médias sociaux est également exclu.
...
AEM a également mis en place des règles pour exclure les robots connus, y compris les services qui visitent régulièrement le site pour actualiser leur index ou service de recherche.
...
### Comment en savoir plus sur le volume de ma demande de contenu ?{#current-request-volumes}

Si vous souhaitez obtenir des informations supplémentaires sur le volume des demandes de contenu affiché dans le tableau de bord Licences , votre équipe Adobe fournit un rapport qui indique les principaux facteurs de volume des demandes de contenu. Pour demander un rapport sur l’utilisation optimale, contactez votre équipe Adobe ou l’assistance clientèle Adobe.
