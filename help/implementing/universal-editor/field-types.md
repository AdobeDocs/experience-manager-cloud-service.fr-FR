...
Un type de composant numérique permet la saisie d’un nombre. Il propose une configuration supplémentaire.

| Configuration | Type de valeur | Description | Requis |
|---|---|---|---|
| `valueFormat` | `string` | Le type de nombre à stocker peut être `long` (par défaut) ou `double`. | Non |

Il propose également d’autres types de validation.
