...
* **Forms adaptatif basé sur XSD :** Vous pouvez utiliser XDP-template pour concevoir un modèle de document à enregistrer. Le service ne prend pas en charge le formulaire adaptatif basé sur XFA
...
* Le modèle de données Forms ne prend en charge que les points d’entrée HTTP et HTTPS pour envoyer des données. Le service ne prend pas en charge le protocole d’authentification SSL mutuelle pour le connecteur REST ni l’authentification par certificat x509 pour les sources de données SOAP.
