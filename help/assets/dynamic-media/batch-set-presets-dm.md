...
Utilisez les **[!UICONTROL paramètres prédéfinis de lot]** pour faciliter la création et l’organisation de différentes ressources dans une visionneuse d’images ou une visionneuse à 360° lorsque vous chargez des fichiers de ressources dans un dossier, individuellement ou par une ingestion en masse. Il est possible d’exécuter le paramètre prédéfini en même temps que les tâches d’importation de ressources planifiées dans [!DNL Dynamic Media]. Chaque paramètre prédéfini est un ensemble d’instructions indépendant possédant un nom unique. Il définit comment construire la visionneuse d’images ou la visionneuse à 360° à l’aide d’images correspondant aux conventions d’affectation des noms définies dans la recette du paramètre prédéfini.

>[!IMPORTANT]
>
>Si vous avez utilisé des paramètres prédéfinis de lot dans [!DNL Dynamic Media Classic] et que vous effectuez une migration entre [!DNL Dynamic Media Classic] et Adobe Experience Manager as a Cloud Service, vous devrez recréer manuellement vos définitions de paramètres prédéfinis de lot dans [!DNL Adobe Experience Manager as a Cloud Service].
...
Idéalement, votre entreprise doit également avoir défini une convention d’affectation des noms pour la manière dont les ressources doivent être regroupées dans un ensemble.
...
Bien qu’un tri séquentiel forcé soit possible, il est généralement préférable de se baser autant que possible sur la numérotation alphanumérique pour le tri séquentiel. En outre, vous pouvez utiliser les outils d’éditeur de visionneuse d’images ou de visionneuse à 360° de [!DNL Dynamic Media] pour réorganiser facilement l’ordre séquentiel des ressources, ou ajouter et supprimer de nouvelles ressources dans la visionneuse par une opération glisser-déposer.
...
1. Dans la boîte de dialogue **[!UICONTROL Créer un paramètre prédéfini de lot]**, dans le champ **[!UICONTROL Nom du paramètre prédéfini]**, saisissez un nom descriptif. Notez que le nom du paramètre prédéfini n’est pas modifiable si vous décidez de le modifier ultérieurement.
...
Voir [Détails du paramètre prédéfini, Définir la convention d’affectation des noms et Résultats de la règle – Options RegX](#features-options-bsp) pour en savoir plus sur les options modifiables disponibles.
...
1. Créez un ou plusieurs groupes d’expressions régulières.
   ...
   * Avec les listes déroulantes et les zones de texte du formulaire du groupe, spécifiez un groupe d’expressions que vous souhaitez utiliser pour définir les critères d’affectation de noms pour les ressources de la visionneuse d’images ou de la visionneuse à 360°.
      * Lorsque vous sélectionnez et spécifiez des expressions pour un groupe, vous remarquerez que la syntaxe d’expression régulière se reflète dans la partie inférieure droite de la page, sous l’en-tête **[!UICONTROL Résultats de la règle – RegX]** (vous devrez peut-être appuyer ailleurs, en dehors de la zone de formulaire, pour voir la chaîne d’expression régulière mise à jour dans la partie inférieure droite). Ces chaînes d’expression régulière représentent le modèle que vous souhaitez mettre en correspondance dans une recherche de ressources [!DNL Dynamic Media] pour créer votre visionneuse d’images ou votre visionneuse à 360°.
...
1. Utilisez l’une des méthodes suivantes :
   ...
   * Examinez la syntaxe d’expression régulière dans la zone **[!UICONTROL Résultats de la règle – RegX]**. Si vous devez modifier la syntaxe, apportez vos modifications au groupe correspondant situé à gauche de la page.
...
Vous êtes maintenant prêt à appliquer le paramètre prédéfini de lot à un ou plusieurs dossiers de ressources, à charger des ressources dans le dossier, puis à créer votre visionneuse d’images ou votre visionneuse à 360°. Voir [À propos de l’application des paramètres prédéfinis de lot aux dossiers](#apply-bsp).
...
| Type | Lecture seule. Le type a été spécifié lors de la première création du lot. La copie d’un paramètre prédéfini de lot existant ne vous permet pas de modifier son [!UICONTROL type] ; vous devez créer un nouveau paramètre prédéfini. |
| Inclure les ressources dérivées | Facultatif. Sélectionnez **[!UICONTROL Oui]** (par défaut) pour que le système IPS (Image Production System) de [!DNL Dynamic Media] incorpore des images générées ou « dérivées » dans la visionneuse à 360° ou la visionneuse d’images. Une ressource dérivée est une image qui n’a pas été directement chargée par un utilisateur. Au lieu de cela, la ressource a été produite par l’IPS lors du chargement d’une ressource principale. Par exemple, une ressource d’image générée par l’IPS à partir d’une page d’un fichier PDF, au moment où le fichier PDF a été chargé dans [!DNL Dynamic Media], est considérée comme une ressource dérivée. |
| Dossier de destination | Facultatif.  Si vous définissez un grand nombre de visionneuses d’images ou de visionneuses à 360°, vous préférerez sans doute les conserver séparément des dossiers contenant les ressources elles-mêmes. Vous pouvez donc envisager de créer un dossier Visionneuses d’images ou Visionneuses à 360° et de rediriger l’application pour y placer les visionneuses créées sous forme de lots.<br>Dans ce cas, spécifiez le dossier dans la structure de dossiers Adobe Experience Manager Assets (`/content/dam`) pour lequel le paramètre prédéfini de lot doit être actif. Assurez-vous que le dossier est activé pour la synchronisation de [!DNL Dynamic Media] afin de l’autoriser en tant que dossier de destination. Voir [Configuration de la publication sélective au niveau des dossiers dans Dynamic Media](/help/assets/dynamic-media/selective-publishing.md#selective-publish-configure-folder).<br>Sachez que plusieurs dossiers peuvent être dotés d’un paramètre prédéfini de lot donné si vous appliquez ce paramètre au moyen des **[!UICONTROL Propriétés]** du dossier. Voir [Application de paramètres prédéfinis de lot à partir de la page Propriétés d’un dossier de ressources](#apply-bsp-to-folders-via-properties).<br>Si vous ne spécifiez pas de dossier, le paramètre prédéfini de lot est créé dans le même dossier que le dossier de téléchargement des ressources. |
| **[!UICONTROL Définir la convention d’affectation des noms]** |  |
| Préfixe<br>ou<br>Suffixe | Facultatif. Entrez un préfixe, un suffixe ou les deux dans les champs respectifs.<br>Les champs de préfixe et de suffixe permettent de créer autant de paramètres prédéfinis de lot à l’aide d’une autre convention personnalisée d’affectation de noms de fichier qui peut s’avérer nécessaire pour un ensemble de contenu particulier. Cette méthode est particulièrement utile dans les cas où il existe une exception à un schéma d’affectation de nom par défaut défini par une société.<br>Le préfixe ou le suffixe est ajouté au **[!UICONTROL nom de base]** que vous définissez dans la zone **[!UICONTROL Convention d’affectation des noms de ressources]**. En ajoutant un préfixe ou un suffixe, vous vous assurez que la visionneuse d’images ou la visionneuse à 360° est créée de manière exclusive et indépendante des autres ressources. Cette opération peut également aider d’autres personnes à identifier les types de fichiers. Par exemple, pour déterminer un mode de couleur utilisé, vous pouvez ajouter comme préfixe ou suffixe `rgb` ou `cmyk`.<br>Bien que la définition d’une convention d’affectation des noms ne soit pas nécessaire pour utiliser la fonctionnalité de paramètre prédéfini de lot, il est recommandé d’utiliser la convention d’affectation des noms fixée pour définir autant d’éléments de votre convention que vous souhaitez regrouper dans une visionneuse afin de mieux rationaliser la création d’un ensemble par lot. |
...
Lorsque vous affectez des paramètres prédéfinis de lot à un ou plusieurs dossiers, tous les sous-dossiers héritent automatiquement des paramètres prédéfinis de leur dossier parent.

Vous pouvez appliquer plusieurs paramètres prédéfinis de lot à un dossier.
...
Utilisez l’une des deux méthodes suivantes pour appliquer des paramètres prédéfinis de lot aux dossiers :
...
Il peut s’avérer nécessaire de retraiter des ressources dans un dossier si vous rencontrez l’une des deux situations suivantes :
...
1. Sur la page **[!UICONTROL Sélectionner le(s) dossier(s)]**, cochez la case de chaque dossier auquel vous voulez appliquer les paramètres prédéfinis de lot.
1. Dans l’angle supérieur droit de la page **[!UICONTROL Sélectionner le(s) dossier(s)]**, appuyez sur **[!UICONTROL Appliquer]**.
...
1. Sous **[!UICONTROL Paramètre(s) prédéfini(s) de lot]**, sélectionnez le nom d’un paramètre prédéfini de lot à appliquer dans la liste déroulante **[!UICONTROL Nom du paramètre prédéfini]**. La capture d’écran ci-dessus montre que deux paramètres prédéfinis de lot ont été appliqués au dossier.
   ...
   Pour supprimer un paramètre prédéfini de lot appliqué, appuyez sur **[!UICONTROL X]** à droite du type de paramètre prédéfini.
...
Toutefois, vous ne pouvez pas modifier le nom ni le type de paramètre prédéfini (visionneuse d’images ou visionneuse à 360°). S’il devient nécessaire de modifier le nom d’un paramètre prédéfini, il suffit de copier le paramètre prédéfini existant et de spécifier un nouveau nom. Voir [Copie d’un paramètre prédéfini de lot](#copy-bsp).

Si vous modifiez un paramètre prédéfini de lot précédemment appliqué à un dossier, ce paramètre n’est appliqué qu’aux nouvelles ressources téléchargées dans le dossier.

Si vous souhaitez que ce paramètre prédéfini modifié soit réappliqué aux ressources existantes du dossier, vous devez retraiter le dossier. <!-- See [Reprocessing assets in a folder](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets). --> Ainsi, les ressources existantes pourraient désormais être incluses dans une visionneuse d’images ou une visionneuse à 360° et y être ajoutées. En outre, les ressources existantes qui étaient déjà incluses dans la visionneuse d’images ou la visionneuse à 360°, selon le paramètre prédéfini de lot précédent utilisé, ne sont pas supprimées (à condition qu’elles ne soient plus qualifiées en fonction du paramètre prédéfini récemment modifié) et s’affichent en l’état.
...
Lorsque vous supprimez des paramètres prédéfinis de lot dans des dossiers, les nouvelles ressources chargées vers ces dossiers ne sont pas associées au paramètre prédéfini de lot. Les ressources existantes du dossier déjà ajoutées à la visionneuse d’images ou à la visionneuse à 360°, en fonction du paramètre prédéfini de lot appliqué au dossier, continueront à s’afficher en l’état.
...
* [Supprimer des paramètres prédéfinis de lot à partir de la page Propriétés d’un dossier](#remove-bsp-from-folders-via-properties). Cette méthode permet de supprimer un ou plusieurs paramètres prédéfinis de lot d’un seul dossier.
...
1. Sur la page **[!UICONTROL Sélectionner le(s) dossier(s)]**, sélectionnez un ou plusieurs dossiers dans lesquels vous souhaitez supprimer les paramètres prédéfinis de lot. La capture d’écran ci-dessus montre un dossier sélectionné avec le nom de deux paramètres prédéfinis de lot qui lui ont déjà été appliqués et qui seront supprimés.
1. Dans l’angle supérieur droit de la page **[!UICONTROL Sélectionner le(s) dossier(s)]**, appuyez sur **[!UICONTROL Supprimer]**.
...
1. Dans la boîte de dialogue **[!UICONTROL Supprimer les paramètres prédéfinis de lot]**, appuyez sur **[!UICONTROL Supprimer]**.
   ...
   Si le paramètre prédéfini que vous supprimez a été référencé à l’aide d’un dossier de ressources, vous devrez peut-être appuyer à la place sur **[!UICONTROL Forcer la suppression]**.
