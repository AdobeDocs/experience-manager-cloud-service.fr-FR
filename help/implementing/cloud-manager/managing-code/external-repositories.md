...
1. Dans la boîte de dialogue **Validation de la propriété du référentiel privé**, fournissez un jeton d’accès pour valider la propriété du référentiel externe afin que vous puissiez y accéder.
   ...
   *Sélection d’un jeton d’accès existant pour un référentiel Bitbucket.*
   ...
   | **Ajout d’un nouveau jeton d’accès** | **Type de référentiel : GitHub**<br><ul><li> Dans le champ de texte **Nom du jeton**, saisissez un nom pour le jeton d’accès que vous êtes en train de créer.<li>Créez un jeton d’accès personnel en suivant les instructions de la [documentation GitHub](https://docs.github.com/fr/enterprise-server@3.14/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens).<li>Pour les autorisations requises, définissez les éléments suivants :<ul><li>**Autorisations requises pour le jeton d’accès personnel (PAT) GitHub**<br> ces autorisations permettent à Cloud Manager de valider les demandes d’extraction, de gérer les vérifications de statut de validation et d’accéder aux détails de référentiel nécessaires. Dans GitHub, lorsque vous générez le jeton d’accès personnel (PAT), assurez-vous qu’il comprend les éléments suivants :<ul><li>**Autorisations de référentiel :**<ul><li>Demande d’extraction (lecture et écriture)<li>Commit status (lecture et écriture).<li>Métadonnées du référentiel (lecture seule)</li></li></ul></ul><li>**Événements webhook obligatoires (pour les référentiels hébergés sur GitHub)**<br> Ces événements permettent à Cloud Manager de répondre à l’activité GitHub, comme la validation des demandes d’extraction, les déclencheurs basés sur les notifications push pour les pipelines ou la synchronisation du code Edge Delivery Services. Lorsque vous configurez manuellement un webhook GitHub, assurez-vous que le webhook est configuré pour se déclencher sur les événements suivants :<ul><li>**Événements webhook requis :**<ol><li>Requêtes d’extraction<li>Notifications push<li>Commentaires sur l&#39;événement</li></li></ul></ul><ul><li>Dans le champ **Jeton d’accès**, collez le jeton que vous venez de créer. |
   |  | **Type de référentiel : GitLab**<ul><li>Dans le champ de texte **Nom du jeton**, saisissez un nom pour le jeton d’accès que vous êtes en train de créer.<li>Créez un jeton d’accès personnel en suivant les instructions de la [documentation GitLab](https://docs.gitlab.com/user/profile/personal_access_tokens/).<li>Pour les autorisations requises, définissez les éléments suivants :<ul><li>**Autorisations requises pour le jeton d’accès personnel GitLab (PAT)**<br> ces portées permettent à Cloud Manager d’accéder aux données du référentiel et aux informations utilisateur selon les besoins pour la validation et l’intégration du webhook. Lors de la création du jeton d’accès personnel dans GitLab, assurez-vous qu’il inclut les portées suivantes :<ul><li>**Portées des jetons :**<ul><li>api<li>read_user</li></li></ul></ul><li>**Événements webhook obligatoires (pour les référentiels hébergés par GitLab)**<br> Ces événements webhook permettent à Cloud Manager de déclencher des pipelines lorsque le code est envoyé ou qu’une demande de fusion est envoyée. Ils effectuent également le suivi des commentaires liés à la validation de la demande d’extraction (par le biais d’événements de note). Lorsque vous configurez manuellement un webhook dans GitLab, veillez à inclure les événements suivants :<ul><li>**Événements webhook requis :**<ol><li>Événements push<li>Événements de demande de fusion<li>Événements de note</li></li></li></ul></ul></ul><ul><li>Dans le champ **Jeton d’accès**, collez le jeton que vous venez de créer. |
   |  | **Type de référentiel : Bitbucket**<ul><li>Dans le champ de texte **Nom du jeton**, saisissez un nom pour le jeton d’accès que vous êtes en train de créer.<li>Créez un jeton d’accès au référentiel à l’aide de la [documentation Bitbucket](https://support.atlassian.com/bitbucket-cloud/docs/create-a-repository-access-token/).<li>Pour les autorisations requises, définissez les éléments suivants :<ul><li>**Autorisations requises pour le jeton d’accès personnel (PAT) Bitbucket**<br> ces autorisations permettent à Cloud Manager d’accéder au contenu du référentiel, de gérer des demandes d’extraction et de configurer des événements webhook ou d’y réagir. Lors de la création du mot de passe de l’application dans Bitbucket, assurez-vous qu’il comprend les éléments suivants :<ul><li>**Autorisations de mot de passe d’application requises :**<ul><li>Référentiel (lecture seule)<li>Requêtes d’extraction (lecture et écriture)<li>Webhooks (lecture et écriture)</li></li></ul></ul><li>**Événements webhook requis (pour les référentiels hébergés Bitbucket)**<br> Ces événements permettent à Cloud Manager de valider les demandes d’extraction, de répondre aux envois de code et d’interagir avec les commentaires pour la coordination du pipeline. Si vous configurez le webhook manuellement dans Bitbucket, configurez-le pour qu’il se déclenche sur les événements suivants :<ul><li>**Événements webhook requis :**<ol><li>Demande d’extraction : créée<li>Demande d’extraction : mise à jour<li>Demandes d’extraction : fusionnées<li>Demande d’extraction : commentaire<li>Référentiel : Push</li></li></li></ul></ul></ul><ul><li>Dans le champ **Jeton d’accès**, collez le jeton que vous venez de créer. |
