...
>abstract="Découvrez comment restaurer votre contenu AEM as a Cloud Service à partir de la sauvegarde à l’aide de Cloud Manager."

Découvrez comment restaurer votre contenu AEM as a Cloud Service à partir de la sauvegarde à l’aide de Cloud Manager.

>[!NOTE]
>
>* Cette fonctionnalité est en cours de déploiement et peut ne pas encore être activée pour tous les clients de Cloud Manager.
>* Cette fonctionnalité est actuellement limitée aux environnements d’évaluation et de développement. L’utilisation des fonctionnalités et les commentaires de ces types d’environnements assurent un déploiement réussi dans les environnements de production dans un avenir proche.
...
* **Point dans le temps (PIT) :** ce type effectue une restauration à l’aide de sauvegardes système continues des dernières 24 heures à partir de l’heure actuelle.
...
Les mesures de performances de restauration du contenu dans AEM as a Content Service se rapportent aux références normalisées :

* **Objectif de temps de récupération (RTO) :** l’objectif de temps de récupération varie en fonction de la taille du référentiel, mais en règle générale, une fois que la séquence de récupération commence, il doit prendre environ 30 minutes.
* **L’objectif du point de récupération (RPO) :** L’objectif du point de récupération dépend du niveau de temps de restauration sélectionné dans le passé :
   * pour les premières 24 heures d&#39;opération : à tout moment pendant ces 24 heures.
   * après les 24 heures initiales : un maximum de 12 heures pour les six jours précédents.
...
>[!NOTE]
>
>Un utilisateur avec le rôle de **Propriétaire de l’entreprise** ou **Responsable de déploiement** doit être connecté pour lancer une opération de restauration.

1. Connectez-vous à Cloud Manager à l’adresse [my.cloudmanager.adobe.com](https://my.cloudmanager.adobe.com/) et sélectionnez l’organisation appropriée.

1. Cliquez sur le programme pour lequel vous souhaitez lancer une restauration.

1. À partir de la page **Aperçu du programme**, dans la vignette **Environnements**, cliquez sur le bouton représentant des points de suspension en regard de l’environnement pour lequel vous souhaitez lancer une restauration et sélectionnez **Restaurer le contenu**.
...
Le processus de sauvegarde est lancé et vous pouvez afficher son statut dans le tableau **[Restaurer l’activité](#restore-activity)**. Le temps nécessaire à l’achèvement d’une opération de restauration dépend de la taille et du profil du contenu restauré.
...
Restaure uniquement le contenu sur AEM. Pour cette raison, vous devez soigneusement tenir compte des modifications de code effectuées entre le point de restauration souhaité et l’heure actuelle en examinant l’historique de validation entre l’ID de validation actuel et celui en cours de restauration.
...
Le tableau **Restaurer l’activité** indique le statut des dix demandes de restauration les plus récentes, y compris les opérations de restauration actives.
