...
La date de publication de la version actuelle (2024.6.0) d’[!DNL Adobe Experience Manager] as a [!DNL Cloud Service] est le mercredi 25 juin 2024. La prochaine disponibilité des fonctionnalités (2024.7.0) est prévue pour le 27 juillet 2023.
...
**Autres options disponibles dans le tableau de bord Assets Insights**

Le décompte des ressources par type et taille de ressource est désormais disponible dans le tableau de bord Statistiques sur Assets . Ces options fournissent, dans votre environnement de vue Assets, des données en temps réel sur le nombre et le pourcentage de ressources par tranche de taille et type de ressource.
...
Le [centre d’actions](/help/operations/actions-center.md) envoie des notifications par e-mail lorsque des incidents importants se produisent ou si nous observons quelque chose sur votre code ou configuration où vous devez prendre des mesures proactives. Nous avons maintenant introduit plusieurs nouveaux types de notifications associées à l’intégrité de votre contenu. Il est disponible par le biais d’un programme d’adoption précoce. Pour participer, contactez l’assistance clientèle Adobe.
...
Les performances du moteur de workflow sont affectées lorsqu’un grand nombre de workflows sont en cours d’exécution dans l’environnement de création. Recevez une notification proactive via le centre d’actions lorsqu’un grand nombre d’instances de workflow en cours d’exécution sont détectées, ce qui vous permet de configurer une tâche de purge pour arrêter les workflows qui ne sont pas requis.

#### Utilisateurs ajoutés directement aux groupes personnalisés {#users-customgroups}

Recevez une notification proactive par l’intermédiaire du centre d’actions lorsque les utilisateurs sont ajoutés directement aux groupes personnalisés, ce qui vous permet de suivre les bonnes pratiques IMS pour ajouter des utilisateurs aux groupes IMS appropriés, puis d’ajouter les groupes IMS en tant que membres de groupes AEM.

#### Contenu JCR manquant {#jcr-content}

Recevez une notification proactive via le centre d’actions lorsque le contenu JCR manquant est détecté, ce qui vous permet d’ajouter le contenu JCR manquant et d’éviter l’échec de certaines fonctionnalités d’AEM Assets.

#### Workflows terminés non purgés {#workflows}

Recevez une notification proactive par le biais d’ Actions Center lorsque les workflows terminés sur 90 jours n’ont pas été purgés, ce qui vous permet d’améliorer les performances de votre moteur de workflow en réduisant le nombre d’instances de workflows.

#### Ressource Sling manquante {#sling-resource}

Recevez une notification proactive par le biais d’ Actions Center lorsqu’une ressource Sling manquante est détectée, ce qui vous permet d’ajouter la ressource Sling manquante et d’éviter l’échec de certaines fonctionnalités d’AEM Assets.
