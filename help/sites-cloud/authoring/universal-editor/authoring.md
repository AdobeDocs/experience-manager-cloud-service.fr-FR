...
Le panneau [propriétés](/help/sites-cloud/authoring/universal-editor/navigation.md#properties-rail) affiche une entrée pour le composant de texte enrichi sélectionné.

![Composant Texte enrichi dans le panneau Propriétés](assets/rich-text-properties-panel.png)

#### Éditeur modal {#modal-editor}

Appuyez sur l’entrée dans [le panneau Propriétés](#properties-rail) pour ouvrir un éditeur modal qui offre une zone de travail plus grande pour modifier le texte enrichi.

![Boîte de dialogue d’édition de texte enrichi](assets/rich-text-canvas.png)

Appuyez ou cliquez sur **Annuler** ou **Terminé** pour ignorer ou enregistrer les modifications, respectivement. Vous pouvez également appuyer sur la touche Échap pour enregistrer les modifications et fermer la boîte de dialogue.

#### Options de formatage de texte enrichi {#formatting-options}

L’éditeur de texte enrichi (RTE) de l’éditeur universel permet à l’auteur d’appliquer une mise en forme de texte standard. Les options suivantes sont disponibles.

* **Style de paragraphe**
   * Paragraphe, h1-h6, code
* **Gras**
* **Italique**
* **Souligné**
* **Barré**
* **Couleur du texte**
   * Ouvre une palette de couleurs dans laquelle vous pouvez sélectionner une couleur ou spécifier une valeur hexadécimale
   * Disponible uniquement dans l’éditeur modal, et non dans le contexte
* **Exposant**
* **Indice**
* **Liste à puces**
...
* **Caractère spécial**
   * Insérez un caractère spécial dans votre texte.
* **Supprimer toute mise en forme**
   * Supprimez toutes les options de mise en forme du texte sélectionné.

Selon votre serveur principal, les options disponibles par défaut peuvent varier. L’éditeur de texte enrichi peut être configuré pour masquer des options ou afficher des options supplémentaires en fonction des besoins des auteurs. Consultez le document [Configuration de l’éditeur de texte enrichi pour l’éditeur universel](/help/implementing/universal-editor/configure-rte.md) pour plus d’informations.
...
1. Vous pouvez ensuite effectuer l’une des opérations suivantes :

   * Sélectionnez l’icône **Ajouter** dans le panneau des propriétés.

     ![Sélectionner l’icône d’ajout](assets/add-icon.png)

   * Sélectionnez l&#39;option **Ajouter** dans le menu contextuel.

     ![Ajouter à partir du menu contextuel](assets/add-from-context-menu.png)

1. La boîte de dialogue du sélecteur de composants s’ouvre.
   * Utilisez la colonne de gauche pour filtrer les composants par catégorie ou utilisez la recherche pour filtrer par nom.
   * Cliquez sur le nom du composant dans la colonne de droite pour l’insérer dans le conteneur.
   * Si un seul composant est autorisé dans le conteneur, il est inséré automatiquement.
   * Cliquez en dehors du sélecteur pour annuler l’insertion du composant.

   ![Sélecteur de composant](assets/component-picker.png)
...
1. Vous pouvez ensuite effectuer l’une des opérations suivantes :
   ...
   * Sélectionnez l’icône **Dupliquer** dans le panneau des propriétés.

     ![Sélection d’un composant à ajouter à un conteneur](assets/ue-duplicate-component.png)

   * Sélectionnez l’option **Dupliquer** dans le menu contextuel.

     ![Dupliquer à partir du menu contextuel](assets/duplicate-from-context-menu.png)

Le composant est dupliqué et inséré sous le composant sélectionné.

### Suppression de composants de conteneurs {#deleting-components}

1. Sélectionnez un composant dans un conteneur dans l’[arborescence de contenu](/help/sites-cloud/authoring/universal-editor/navigation.md#content-tree-mode) ou dans l’éditeur.
1. Vous pouvez ensuite effectuer l’une des opérations suivantes :
   * Sélectionnez l’icône **Supprimer** dans le panneau des propriétés.

     ![Supprimer un composant](assets/ue-delete-component.png)
   * Sélectionnez l’option **Supprimer** dans le menu contextuel.
     ![Supprimer du menu contextuel](assets/delete-from-context-menu.png)
...
### Réorganisation et déplacement des composants {#reordering-components}

Vous pouvez déplacer et réorganiser les composants par glisser-déposer, à l’aide du menu contextuel ou de l’arborescence de contenu.
...
#### Déplacer des composants par glisser-déposer {#drag-and-drop-move}

1. Cliquez sur le composant à déplacer et faites-le glisser.
   * Lorsque vous placez le pointeur de la souris sur la destination, l’éditeur vous indique où sera placé le composant si vous le déposez avec une ligne bleue horizontale.
     ![Faites glisser et déposez un composant pour le déplacer](assets/drag-and-drop-component-move.png)
1. Déposez le composant pour le déplacer.

#### Déplacer des composants avec le menu contextuel {#move-context-menu}

1. Cliquez avec le bouton droit de la souris sur un composant ou cliquez sur le badge du composant sélectionné pour ouvrir le [menu contextuel](#context-menu).
1. Sélectionnez l’option de déplacement souhaitée.
...
>[!TIP]
>
>Utilisez les touches de raccourci `Command-U` ou `Shift-Command-U` pour vous déplacer vers le haut ou vers le haut, respectivement.
>Utilisez les touches de raccourci `Command-J` ou `Shift-Command-J` pour descendre ou descendre vers le bas, respectivement.

>[!NOTE]
>
>Les options de menu contextuel ne peuvent déplacer que des composants dans leurs conteneurs. Si vous souhaitez déplacer des composants entre des conteneurs, [utilisez l’arborescence de contenu](#reorder-content-tree).

#### Déplacer des composants avec l’arborescence de contenu {#reorder-content-tree}
...
1. Vous pouvez ensuite effectuer l’une des opérations suivantes :
   * Cliquez sur l’icône **Copier** dans le panneau [Propriétés.](/help/sites-cloud/authoring/universal-editor/navigation.md#properties-panel)
     ![Copier à partir du panneau](assets/copy-from-panel.png)
   * Sélectionnez l’option **Copier** dans le menu contextuel.
     ![Copier à partir du menu contextuel](assets/copy-from-context-menu.png)
1. Sélectionnez le composant _après_ auquel vous souhaitez coller le composant copié.
1. Vous pouvez ensuite effectuer l’une des opérations suivantes :
   * Appuyez ou cliquez sur **Coller** dans le panneau des propriétés.
     ![Coller à partir du panneau](assets/paste-from-panel.png)
   * Sélectionnez **Coller** dans le menu contextuel.
     ![Coller à partir du menu contextuel](assets/paste-from-context-menu.png)

Le composant copié est collé _après_ le composant sélectionné.
