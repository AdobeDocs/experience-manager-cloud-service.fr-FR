...
Les fournisseurs de services de traduction suivants fournissent une implémentation de l’API AEM pour les projets de traduction.
...
* [Classer les technologies pour tablettes](https://exchange.adobe.com/experiencecloud.details.90064.clay-tablet-translation-for-experience-manager.html)
...
* Quel fournisseur de services de traduction utiliser
* Si la traduction humaine ou automatique doit être effectuée
...
1. Dans le [menu de navigation globale,](/help/sites-cloud/authoring/getting-started/basic-handling.md#global-navigation) cliquez ou appuyez sur **Outils -> Cloud Services -&amp; Cloud Services de traduction**.
...
1. Indiquez les informations suivantes dans les champs, puis cliquez ou appuyez sur **Créer** :
   ...
   1. Vous pouvez éventuellement saisir un **nom** à utiliser pour le noeud de référentiel qui stocke la configuration.
...
L’onglet **Sites** contrôle la manière dont la traduction du contenu de la page est effectuée.

| Propriété | Description |
|---|---|
| Processus de traduction | Cette propriété définit la méthode de traduction que la structure exécute pour le contenu du site :<br>- Traduction automatique : Le fournisseur de traduction effectue la traduction à l’aide de la traduction automatique en temps réel.<br>- Traduction humaine : le contenu est envoyé au fournisseur de services de traduction pour être traduit par des traducteurs.<br>- Ne pas traduire : Le contenu n’est pas envoyé pour traduction. Ceci permet d’ignorer certaines parties du contenu qui n’ont pas besoin d’être traduites, mais dont le contenu pourrait être devoir être mis à jour. |
| Fournisseur de traduction | Cette propriété définit le fournisseur de traduction pour effectuer la traduction. Un fournisseur apparaît dans la liste lorsque son connecteur correspondant est installé. |
...
| Traduire les balises | Cette option active la traduction des balises associées à la page. |
| Traduire les ressources de page | Cette propriété définit la manière de traduire les ressources ajoutées aux composants à partir du système de fichiers ou référencées à partir des ressources :<br> - Ne pas traduire : Les ressources de page ne sont pas traduites.<br>- Utilisation du workflow de traduction des sites : Les ressources sont gérées en fonction des propriétés de configuration de l’onglet  **** Sites .<br>- Utilisation du workflow de traduction des ressources : Les ressources sont gérées en fonction des propriétés configurées dans l’onglet  **** Ressources. |
...
| Propriété | Description |
|---|---|
| Processus de traduction | Cette propriété sélectionne le type de traduction que la structure effectue pour les ressources :<br>- Traduction automatique : Le fournisseur de traduction effectue immédiatement la traduction à l’aide de la traduction automatique.<br>- Traduction humaine : le contenu est envoyé automatiquement au fournisseur de services de traduction pour être traduit manuellement.<br>-Do Not Translate: Les ressources ne sont pas envoyées pour traduction. |
| Fournisseur de traduction | Cette propriété définit le fournisseur de traduction pour effectuer la traduction. Un fournisseur apparaît dans la liste lorsque son connecteur correspondant est installé. |
| Catégorie de contenu | (Traduction automatique uniquement) Cette propriété décrit le contenu que vous traduisez. Lors de la traduction du contenu, la catégorie peut affecter le choix de la terminologie et des expressions. |
