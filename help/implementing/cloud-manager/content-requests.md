...
| Robots AI/LLM bien connus (identifiés par Fastly) | Exclu | Demandes provenant de robots AI/LLM reconnus qui sont identifiés comme des robots bien connus (par exemple, par des signaux de classification de robots `User-Agent` ou autres). Ces demandes ne sont pas facturables.<br><br>Si un agent d’IA n’est pas identifié comme un robot bien connu (par exemple, il utilise un `User-Agent` de navigateur générique), ses requêtes peuvent être comptabilisées comme des requêtes de contenu facturables. |
...
* Trouvez l’équilibre entre vos besoins de fraîcheur de contenu et le coût de licence AEM pour tout moteur de recherche personnalisé que vous avez intégré à votre site.  Un robot d’exploration trop agressif peut consommer de nombreuses requêtes de contenu.
