...
>[!TIP]
>
>Il est recommandé de minimiser le nombre de balises qui se rapportent aux mêmes idées. Par exemple, si vous gérez du contenu pour un magasin d’approvisionnement en plein air, vous n’avez probablement pas besoin d’une balise pour les deux **chaussures** et **chaussures**.
...
* Le nom d’une balise ne peut pas inclure de délimiteurs de balise :
   * Deux-points (`:`) - Détermine la balise d’espace de noms
...
* Les balises peuvent être créées et leur taxonomie peut être modifiée par les membres de la `tag-administrators` groupe et membres disposant de droits de modification sur `/content/cq:tags`.
...
1. [Publiez la balise.](#publishing-tags)
...
1. Pour créer une balise, ouvrez le [console de balisage.](#tagging-console)
...
1. La variable **Créer une balise** s’ouvre. Fournissez les informations requises pour la nouvelle balise.
   ...
   * **Nom** - Nom de la balise (obligatoire). Si aucun nom n’est spécifié, un nom de noeud valide est créé à partir de la propriété **Titre**. Voir [ID de balise](/help/implementing/developing/introduction/tagging-framework.md#tagid).
   * **Description** - Description de la balise
   * **Chemin d’accès aux balises** : valeur par défaut de l’espace de noms (ou de la balise) sélectionné dans la console de balisage. Vous pouvez le mettre à jour manuellement en appuyant ou en cliquant sur l’icône du sélecteur de chemin.
...
1. L’action de publication est confirmée par une **Succès** boîte de dialogue.

   ![Boîte de dialogue de succès de la balise de publication](assets/publish-tag-success.png)
...
Vous pouvez afficher plus de détails sur la balise, y compris le dernier utilisateur à l’avoir publiée et le moment où vous la modifiez en sélectionnant **Propriétés** vue.

1. Pour afficher les propriétés d’une balise, ouvrez le [console de balisage.](#tagging-console)

1. Sélectionnez la balise dont vous souhaitez afficher les propriétés, puis, dans le rail de gauche, sélectionnez **Propriétés**.

   ![Sélection de la vue des propriétés](assets/view-tag-properties.png)
...
1. Pour modifier une balise, ouvrez le [console de balisage.](#tagging-console)
...
Si les modifications que vous avez apportées à une balise déjà publiée, vous pouvez souhaiter [republiez-le.](#publishing-tags)
...
1. Pour annuler la publication d’une balise, ouvrez le [console de balisage.](#tagging-console)
...
1. L’action d’annulation de la publication est confirmée par une **Succès** boîte de dialogue.

   ![Boîte de dialogue de succès de la balise de publication](assets/unpublish-tag-success.png)
...
Il peut s’avérer utile de déterminer le contenu appliqué à une balise particulière. Pour ce faire, utilisez l’option **Références** dans la console de balisage.

1. Pour afficher les références d’une balise, ouvrez le [console de balisage.](#tagging-console)

1. Sélectionnez la balise dont vous souhaitez afficher les références, puis, dans le rail de gauche, sélectionnez **Références**.

   ![Sélection de la vue des propriétés](assets/view-tag-references.png)
...
Si votre taxonomie de balisage comporte des doublons ou des balises similaires, il peut être utile de les fusionner. Lorsque la balise `A` est fusionné dans la balise `B`, toutes les pages balisées avec une balise `A` devenir balisé avec une balise `B` et balise `A` n’est plus disponible pour les auteurs.
...
1. Dans le **Allemand** qui s’affiche, indiquez le titre traduit.
...
Toutefois, dans l’environnement de création, AEM utilise le paramètre de langue de l’utilisateur. Ainsi, dans la console de balisage, pour le `Animals` balise, `Animaux` s’affiche pour un utilisateur qui définit la langue sur le français dans ses propriétés.

Pour ajouter une nouvelle langue à la boîte de dialogue, consultez le document . [Création de balises dans des applications AEM](/help/implementing/developing/introduction/tagging-applications.md#adding-a-new-language-to-the-edit-tag-dialog)
