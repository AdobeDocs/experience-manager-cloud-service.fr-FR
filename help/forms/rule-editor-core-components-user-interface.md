...
Les règles peuvent parfois être rompues si un champ référencé est déplacé ou supprimé. Dans ce cas, l’éditeur de règles marque la règle concernée avec un indicateur de point rouge.

* Lorsqu’un champ est déplacé (par exemple, d’un panneau à un autre), la règle associée peut être rompue. Dans ce cas, ouvrez la règle et enregistrez-la à nouveau. La règle est automatiquement restaurée.

* Lorsqu’un champ est supprimé, la règle ne peut pas être réparée.

  Pour comprendre le problème, pointez sur le statut de la règle dans l’éditeur de règles pour afficher le message d’erreur.
