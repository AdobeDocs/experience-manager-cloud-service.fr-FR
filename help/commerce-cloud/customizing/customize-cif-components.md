...
1. Mettez la requête GraphQL à jour pour inclure l’attribut `eco_friendly` dans la requête partielle :
   ...
   Dans le code ci-dessus, `addCustomSimpleField` est utilisé pour récupérer l’attribut `eco_friendly`. Cela illustre la manière dont vous pouvez effectuer des requêtes pour tout attribut personnalisé faisant partie du schéma Adobe Commerce.
