...
1. Utilisez la variable **Marque** menu déroulant pour sélectionner la marque pour laquelle vous souhaitez créer l’activité.
   ...
   >[!NOTE]
   >
   >Il est recommandé de [créer des marques dans la console activités](/help/sites-cloud/authoring/personalization/activities.md#creating-a-brand-using-the-activities-console).
   >
   >
...
Offres qui sont [créée sur la page](#adding-a-custom-offer) et utilisés pour une seule expérience sont appelés offres personnalisées. L’image suivante est superposée sur le contenu d’une offre personnalisée :
...
Utilisation de l’étape Créer de [le processus de ciblage ;](#the-targeting-process-create-target-and-goals-settings), vous pouvez ajouter et supprimer des expériences. En outre, vous pouvez dupliquer une expérience et la renommer.
...
* [Ciblage d’un composant existant](#creating-a-default-offer-by-targeting-an-existing-component). Le contenu devient l’offre de l’expérience par défaut.
...
* [Ajout d’offres personnalisées](#adding-a-custom-offer).
* [Ajout d’offres à partir d’une bibliothèque](#adding-an-offer-from-an-offer-library).
...
* [Convertir une offre de bibliothèque en offre personnalisée](#converting-a-library-offer-to-a-custom-library).
...
Effectuez la procédure suivante après [lancer le processus de ciblage ;](#the-targeting-process-create-target-and-goals-settings).
...
Ajoutez une offre personnalisée au [bibliothèque d&#39;offres](/help/sites-cloud/authoring/personalization/offers.md) lorsque vous souhaitez le réutiliser comme offre pour plusieurs expériences. Vous pouvez ajouter des offres à la bibliothèque de la marque actuelle que vous ciblez.
...
Pour plus d’informations sur les moteurs de ciblage, voir [Moteur de ciblage](/help/sites-cloud/authoring/personalization/overview.md#targeting-engine).
...
* Le [Activités activées](/help/sites-cloud/authoring/personalization/activities.md).
* Le [règles qui définissent les segments](/help/sites-cloud/authoring/personalization/segmentation.md).
* Contenu des expériences dans les composants Target.
* Le [configuration du moteur de ciblage](/help/sites-cloud/authoring/personalization/activities.md).
...
| Inclure les segments résolus | Si vous cochez cette case, tous les segments résolus dans l’appel de mbox et les paramètres configurés dans la page et dans l’infrastructure sont inclus. Cela ne fonctionne que dans les situations avec l’API XML où vous synchronisez AEM segments. Si des segments dans AEM ne sont pas gérés par Adobe Target (comme les segments de script), cette option vous permet de résoudre le segment dans AEM et d’envoyer à Adobe Target des informations indiquant que le segment est principal. |
