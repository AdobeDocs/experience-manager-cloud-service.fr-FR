...
Les coordonnées de recadrage intelligent dépendent du rapport d’aspect (L/H). Pour les paramètres de recadrage intelligent d’un Profil d’image, si le format est le même pour les dimensions ajoutées dans le Profil d’image, le même format est envoyé à Dynamic Media. Adobe vous recommande d’utiliser la même zone de récolte. Cela permet de s’assurer que les différentes dimensions utilisées dans le Profil d’images n’ont aucun impact.

Chaque génération de recadrage dynamique créée nécessite un traitement supplémentaire. Par exemple, l’ajout de plus de cinq proportions de recadrage dynamique entraîne un taux d’assimilation lent des ressources. Il peut également augmenter la charge sur les systèmes. Étant donné que le recadrage intelligent s’applique aux dossiers, Adobe vous recommande de l’utiliser *uniquement* pour les dossiers où cela est nécessaire.
...
Les dossiers auxquels un profil est affecté sont indiqués dans l’interface utilisateur et le nom du profil apparaît dans la carte.
...
### Application d’un Profil d’images Dynamic Media globalement {#applying-an-image-profile-globally}

Outre l’application d’un profil à un dossier, vous pouvez en appliquer un globalement. Le profil sélectionné est appliqué à tout contenu téléchargé dans les ressources du Experience Manager dans n’importe quel dossier.
