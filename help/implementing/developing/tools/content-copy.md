...
description: L’outil de copie de contenu permet aux utilisateurs de copier du contenu modifiable à la demande à partir de leurs environnements de production sur AEM as a Cloud Service vers des environnements inférieurs à des fins de test.
...
Un ensemble de contenu définit le contenu à copier. Un jeu de contenu se compose d’une liste de chemins JCR. Ces chemins contiennent le contenu modifiable à copier d’un environnement source de service de création vers un environnement cible de service de création, le tout dans le même programme Cloud Manager. Les chemins d’accès suivants sont autorisés dans un ensemble de contenu :
...
1. Si vous devez affiner ou limiter votre jeu de contenu, les sous-chemins peuvent être exclus.
   ...
   1. Dans la liste des chemins d’accès inclus, cliquez sur l’option **Ajouter des sous-chemins à exclure** en regard du chemin que vous souhaitez restreindre.
   1. Saisissez le sous-chemin d’accès à exclure du chemin d’accès sélectionné.
...
>[!NOTE]
> N’utilisez pas la copie de contenu dans un environnement lorsqu’une opération [transfert de contenu](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/overview-content-transfer-tool.md) est en cours d’exécution dans cet environnement.
...
1. Sélectionnez un jeu de contenu dans la console.

1. Dans le menu représentant des points de suspension, cliquez sur **Copier le contenu**.

   ![Copier le contenu](assets/copy-content.png)

   >[!NOTE]
   >
   >Un environnement ne peut pas être sélectionné si l’une des conditions suivantes est vraie :
   >
...
### Journaux d’accès {#accessing-logs}
...
1. Sur la page **Activité de copie de contenu**, dans le menu représentant des points de suspension, cliquez sur **Logs** pour le processus de copie que vous souhaitez réviser. Sélectionnez ensuite l’environnement.

![Accès aux journaux pour le processus de copie de contenu](assets/copy-content-logs.png)

Les journaux sont téléchargés sur votre ordinateur local.

1. Si le téléchargement ne commence pas, vérifiez les paramètres de votre bloqueur de pop-up.
