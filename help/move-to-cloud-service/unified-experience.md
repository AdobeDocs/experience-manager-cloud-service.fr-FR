...
* Unifie les outils qui fonctionnent sur le code source dans une application `node.js` exposée en tant que module externe `aio-cli ` afin de fournir une expérience utilisateur cohérente à l’utilisateur.
...
Le module externe `aio-cli-plugin-aem-cloud-service-migration` se compose de deux parties principales :
...
Reportez-vous à [Ressource Git : aio-cli-plugin-aem-cloud-service-migration](https://github.com/adobe/aio-cli-plugin-aem-cloud-service-migration) pour en savoir plus sur l’utilisation et comment vous pouvez contribuer à ce code de module externe ouvert dans GitHub.
