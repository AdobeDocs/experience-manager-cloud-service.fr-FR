...
Outre l’inclusion dans le journal d’ingestion, le rapport de validation est également accessible à partir de l’interface utilisateur de l’outil de transfert de contenu. Pour cela, sélectionnez un jeu de migration, puis cliquez sur le bouton **Valider** à partir de la barre d’actions :
...
Utilisez le bouton **Rapport de validation Publication/Auteur** pour afficher le rapport de validation de l’ingestion la plus récente au niveau donné de votre environnement cible. Consultez ci-dessous un exemple tiré d’une petite ingestion de publication :

![image](/help/journey-migration/content-transfer-tool/assets/CTTvalidationreport.png)

>[!NOTE]
>
>Le lien vers le **Rapport de validation Publication/Auteur** s’affiche une fois l’ingestion terminée. De plus, les rapports de validation sont conservés, de sorte qu’ils n’expirent pas une fois l’ingestion terminée, comme le font les journaux d’ingestion.
