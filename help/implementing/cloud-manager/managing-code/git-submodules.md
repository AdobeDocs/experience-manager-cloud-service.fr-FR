---
title: Prise en charge des sous-modules Git pour les référentiels Adobe
...
Ajoutez des sous-modules pour les deux autres référentiels :
...
* L’URL Git doit respecter la syntaxe décrite dans la section précédente.
...
* Les références des sous-modules Git sont stockées vers des validations Git spécifiques. Lorsque des modifications sont apportées au référentiel de sous-module, la validation référencée doit être mise à jour.
...
La prise en charge des sous-modules Git dans les [référentiels privés](private-repositories.md) est similaire à leur utilisation avec les référentiels Adobe.

Toutefois, pour que Cloud Manager reconnaisse la configuration du sous-module, ajoutez un fichier `.gitmodules` au répertoire racine du référentiel d’agrégation après avoir configuré votre fichier `pom.xml` et exécuté les commandes `git submodule`.
