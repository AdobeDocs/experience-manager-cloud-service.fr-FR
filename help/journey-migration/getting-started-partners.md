...
Adobe Experience Manager (AEM) as a Cloud Service offre une architecture mise à jour pour Experience Manager. Cette base repose sur une infrastructure basée sur les conteneurs, un développement piloté par les API et un processus DevOps guidé. Cela permet aux spécialistes du marketing et aux développeurs de devancer les innovations de gestion de l’expérience client.

Cloud Service réunit de riches fonctionnalités prêtes à l’emploi et une extensibilité de Adobe Experience Manager, avec l’agilité de l’architecture cloud moderne native, ce qui permet aux marques de répondre à une demande des consommateurs en constante évolution.

Cette page décrit l’approche par étapes recommandée pour transférer les clients des déploiements Experience Manager précédents vers les as a Cloud Service Experience Manager. La nouvelle plateforme personnalisée vous permet de proposer des expériences connectées et continues.
...
Les principes de base du développement du code dans Adobe Experience Manager as a Cloud Service sont similaires à ceux des solutions Adobe Experience Manager On Premise et Managed Services.

Les développeurs écrivent du code et le testent localement, puis le transmettent à des environnements Adobe Experience Manager as a Cloud Service distants.
...
Les développeurs écrivent du code et le testent localement, puis le transmettent à des environnements AEM as a Cloud Service distants.

Cloud Manager, qui était un outil de diffusion de contenu facultatif pour Managed Services, est désormais requis. Il s’agit du seul mécanisme de déploiement de code dans les environnements AEM as a Cloud Service.
...
4. **Accès aux journaux**
   ...
   * [via l’interface de ligne de commande Adobe I/O](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/logs.html#debugging)
5. [Exploitation et maintenance](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/operations/home.html)
   * [Configuration d’OSGI](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/deploying/configuring-osgi.html)
   * [Sauvegarde et restauration](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/operations/backup.html)
...
1. [Conseils et astuces sur le débogage](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/overview.html#debugging-aem-as-a-cloud-service)
...
4. [Journaux et journalisation](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/logs.html#debugging)
   * [Journaux CM](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/build-and-deployment.html#debugging) (test d’unité de construction, analyse de code, image de construction, déploiement)
   * [Journaux Experience Manager Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/logs.html#debugging) (aemerror, aemaccess, aemrequest, aemdispatcher, httpderror, httpaccess)
...
Cloud Manager permet aux entreprises de gérer elles-mêmes Experience Manager sur le cloud. Il comprend une structure d’intégration et de diffusion continues ([CI/CD](https://experienceleague.adobe.com/docs/experience-manager-cloud-manager/content/overview/ci-cd-pipelines.html)) qui permet aux équipes informatiques et aux partenaires d’implémentation d’accélérer la diffusion des personnalisations ou des mises à jour sans compromettre les performances ou la sécurité.
...
2. [Package Manager](https://experienceleague.adobe.com/docs/experience-manager-65/administering/contentmanagement/package-manager.html#package-manager) : permet d’importer et d’exporter du contenu modifiable du référentiel.
...
3. [Déploiement sur l’environnement de développement](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/build-and-deployment.html#debugging)
...
|------------|-------------|---------------------------------|-------------------|
| <ul><li>[Analyseur de bonnes pratiques](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/best-practices-analyzer/overview-best-practices-analyzer.html?lang=fr)</li></li> | <ul><li>[Plug-in Expérience unifiée](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/migration-journey/refactoring-tools/unified-experience.html)</li></ul> | <ul><li>[Les modèles statiques en modèles modifiables](https://opensource.adobe.com/aem-modernize-tools/pages/structure.html)</li><li>[Les configurations de la conception en politiques](https://opensource.adobe.com/aem-modernize-tools/pages/policy.html) <li>[Les composants de base en composants principaux](https://opensource.adobe.com/aem-modernize-tools/pages/component.html)</li><li>[L’interface utilisateur classique en interface utilisateur tactile](https://opensource.adobe.com/aem-modernize-tools/pages/all-in-one.html)</li></ul> | <ul><li>[Outil de transfert de contenu](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/content-transfer-tool/getting-started-content-transfer-tool.html?lang=fr)</li><li>[Gestionnaire de packages](https://experienceleague.adobe.com/docs/experience-manager-65/administering/contentmanagement/package-manager.html#contentmanagement)</li></ul> |
