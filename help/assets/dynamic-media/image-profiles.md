...
>[!NOTE]
>
>Le masque flou n’est appliqué qu’aux rendus réduits au sein du PTIFF (pyramid tiff) dont la résolution est réduite de plus de 50 %. Le masquage flou n’affecte pas les rendus de la plus grande taille du fichier ptiff. En revanche, les rendus de plus petites tailles, tels que les miniatures, sont modifiés (et affichent l’accentuation).
...
<table>
 <tbody>
  ...
  <tr>
   <td>Seuil</td>
   <td><p>Détermine la plage de contraste à ignorer lorsque le filtre de masquage flou est appliqué. Cette option contrôle la différence entre les pixels accentués et leur environnement pour être considérés comme des pixels de contour. Seuls les pixels qui atteignent ce seuil sont accentués. Pour éviter de créer du bruit, essayez des valeurs comprises entre 0 et 255.</p> </td>
  </tr>
 </tbody>
</table>
...
Chaque génération de recadrage intelligent créée nécessite un traitement supplémentaire. Par exemple, l’ajout de plus de cinq proportions de recadrage intelligent peut ralentir le taux d’ingestion des ressources. Cet ajout peut également augmenter la charge des systèmes. Comme le recadrage intelligent peut être appliqué au niveau du dossier, Adobe vous conseille de l’utiliser uniquement pour les dossiers où cela est nécessaire.
...
1. Sélectionnez **[!UICONTROL Appliquer le profil de traitement aux dossiers]** et sélectionnez le ou les dossiers que vous souhaitez utiliser pour recevoir les ressources nouvellement chargées, puis sélectionnez **[!UICONTROL Appliquer]**. Les dossiers auxquels un profil est affecté affichent le nom du profil directement sous celui du dossier.
...
1. Sur la page **[!UICONTROL Modifier les recadrages intelligents]**, utilisez l’une des méthodes suivantes :
   ...
   * Filtrez la liste des images visibles en fonction des noms de point d’arrêt. Dans l’exemple ci-dessous, les images sont filtrées sur le nom de point d’arrêt &quot;Medium&quot;.
