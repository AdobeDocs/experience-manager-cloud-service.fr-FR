...
Vous trouverez ci-dessous un résumé des améliorations continues de la version de maintenance 21994, publiée le 19 août 2025. La version de maintenance précédente était la version 21772.
...
* GRANITE-60183 : commons-fileupload 1.6.0 d’Apache.
* GRANITE-60306 : commons-lang d’Apache vers la version 3.18.0.
* GRANITE-60637 : commons-codec d’Apache vers la version 1.19.0.
* GRANITE-60645 : Apache commons-io 2.20.0.
* GRANITE-60663 : commons-text 1.14.0 d’Apache.
* GRANITE-60714 : pilote Java Mongo 5.2.
* GRANITE-60778 : Filevault 4.0.0.
* GRANITE-60823 : Jackrabbit 2.22.2.
* GRANITE-60967 : création de mesures pour le suivi du temps de compilation clientlib.
* SKYOPS-105469 : ajout de la prise en charge d’acsredirectMgr dans l’API autofix.
* SKYOPS-113929 : ajout de mesures pour la vérification de l’état de préparation de la réplication.
* SKYOPS-84821 : moteur Sling 2.16.6.
* SKYOPS-114322 : augmentation du langage du compilateur de fermeture au niveau d’`ECMASCRIPT_2018`.
...
* GUIDES-30900 : la copie d’un dossier contenant un grand nombre de ressources à partir de l’UI d’Assets entraîne un délai d’expiration de l’API. L’opération continue de s’exécuter sur le serveur principal et se termine au bout d’un certain temps, mais aucun message de réussite ou d’échec, ni aucune notification n’est affiché dans l’UI.
* GUIDES-29090 : dans la sortie PDF native, la liste des index (LOI) s’affiche dans un ordre non alphabétique et les termes d’index imbriqués ne sont pas correctement regroupés, ce qui rend l’index difficile à parcourir.
* GUIDES-11227 : la copie d’un map DITA à partir de l’UI d’Assets copie également sa ligne de base associée dans le nouveau map.
* GUIDES-31506 : la page d’accueil apparaît vide lorsque l’un des fichiers répertoriés dans le widget Fichiers récents est basé sur un modèle dont le modèle source n’inclut pas de miniature.
...
* La version 2.4.65 d’Apache HTTPD introduit des modifications qui peuvent affecter certaines configurations en raison de nouvelles restrictions implémentées dans le cadre de correctifs de sécurité. Ces correctifs corrigent des vulnérabilités en s’assurant que les directives telles que `RequestHeader set`, `edit` et `edit_r` utilisées pour modifier l’en-tête Content-Type sont désormais correctement limitées aux en-têtes de requête. Cette modification empêche toute modification involontaire des en-têtes de réponse, en particulier pour le contenu statique.
...
| Composants principaux AEM | 2.29.0 | [Composants principaux de la gestion de contenu web d’AEM](https://github.com/adobe/aem-core-wcm-components) |
| Node.js | 14 (par défaut) | [Versions de Node.js prises en charge](https://experienceleague.adobe.com/fr/docs/experience-manager-cloud-service/content/implementing/developing/developing-with-front-end-pipelines#node-versions) |
