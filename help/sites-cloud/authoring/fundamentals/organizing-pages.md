...
>[!NOTE]
>
>Votre compte a besoin des droits d’accès] et des autorisations appropriés pour agir sur des pages telles que créer, copier, déplacer, modifier et supprimer.
>
...
La d’un site Web est construite comme une arborescence qui contient vos pages de contenu. Les noms de ces pages de contenu sont utilisés pour former les URL, tandis que les titres sont affichés lorsque le contenu de la page est affiché.

Voici un exemple tiré du site du didacticiel [](https://docs.adobe.com/content/help/en/experience-manager-learn/getting-started-wknd-tutorial-develop/overview.html) WKND, dans lequel un article sur skateparks ( `la-skateparks`) est accessible :
...
>[!NOTE]
>
>Les noms de page sont limités à 150 caractères.
...
A **Title** field containing invalid characters will be accepted, but the name derived will have the invalid characters substituted. Par exemple :
...
Les composants sont les éléments fournis par AEM afin que vous puissiez ajouter des types de contenu spécifiques. AEM est fourni avec une gamme de composants prêts à l’emploi qui fournissent des fonctionnalités complètes. Il s’agit notamment :
...
A moins que toutes les pages n’aient été créées à votre avance, vous devez créer une page avant de commencer à créer du contenu :
...
1. Cliquez ou appuyez sur **Créer** pour terminer le processus et créer la page. La boîte de dialogue de confirmation vous demande si vous souhaitez **ouvrir** immédiatement la page ou revenir à la console (**Terminé**) : 
   ...
   ![Réussite de la création de pages](/help/sites-cloud/authoring/assets/organizing-create-page-success.png)

   >[!NOTE]
   >
   >Si vous créez une page en utilisant un nom qui existe déjà à cet emplacement, le système génère automatiquement une variante du nom en y ajoutant un numéro. For example if `beach` already exists a new page will become `beach1`.
...
Après avoir créé une page ou accédé à une page existante (dans la console), vous pouvez l’ouvrir pour modification :
...
1. À l’étape **Sélectionner la destination** de l’assistant, effectuez l’une des opérations suivantes :
   ...
   * Utilisez la commande **Précédent** pour revenir à l’étape de spécification du nom de la page.
   ...
   ![Sélectionner la destination du déplacement de page](/help/sites-cloud/authoring/assets/move-page-destination.png)
...
1. Si la page est liée ou référencée, ou si elle a été publiée, les détails seront répertoriés dans l’étape **Adapter/Republier**.
   ...
   ![Republier la page au déplacement](/help/sites-cloud/authoring/assets/move-page-republish.png)

1. Sélectionnez **Déplacer** pour terminer le processus et déplacer ou renommer votre page.

>[!NOTE]
>
>Si la page a déjà été publiée, le déplacement de la page en annule automatiquement la publication. By default, it will be republished when the move is complete, but this can changed by un-checking the **Republish** field in the **Adjust/Republish** step.
...
>[!NOTE]
>
>Une page peut uniquement être déplacée vers un emplacement où le modèle sur lequel la page est basée est autorisé. Voir Disponibilité des modèles pour plus d’informations.
<!--
...
1. En [mode de sélection](/help/sites-cloud/authoring/getting-started/basic-handling.md#viewing-and-selecting-resources), sélectionnez la page requise, puis utilisez la commande **Supprimer** de la barre d’outils :
   ...
   ![Bouton de suppression](/help/sites-cloud/authoring/assets/delete.png)
...
>[!NOTE]
>
>Si une page est déjà publiée, elle sera automatiquement annulée avant suppression.

### Verrouillage d’une page {#locking-a-page}

Vous pouvez [verrouiller ou déverrouiller une page](/help/sites-cloud/authoring/fundamentals/editing-content.md#locking-a-page) à partir d’une console ou lorsque vous modifiez une page. Les deux environnements indiquent également si une page est ou non verrouillée.

![Bouton](/help/sites-cloud/authoring/assets/lock.png)Verrouiller![Déverrouiller](/help/sites-cloud/authoring/assets/unlock.png)
