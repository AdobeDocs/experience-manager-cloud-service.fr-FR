...
* Une erreur occasionnelle est désormais corrigée, en raison de laquelle toutes les étapes d’exécution d’un pipeline sont marquées comme terminées. Cependant, le statut du pipeline était toujours en cours d’exécution, ce qui donnait l’impression d’un état bloqué.
