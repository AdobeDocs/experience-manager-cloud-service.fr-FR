...
| Fonctionnalité | Description | [!DNL Workfront] et [!DNL Assets Essentials] *Aucun connecteur (OOTB)* | [!DNL Workfront for Experience Manager enhanced connector] *Nécessite Connecteur* | Workfront et [!DNL Experience Manager as a Cloud Service] *Aucun connecteur (OOTB)* |
|----|----|----|------|-----|-----|
...
| Recherche de dossiers AEM dans Workfront | Le sélecteur AEM Assets dans Workfront permet de rechercher des dossiers en texte intégral. | ✓ | ✓ | ✓ |
...
| Créer automatiquement des dossiers AEM liés dans Workfront | Créez automatiquement des dossiers d’AEM liés dans Workfront à l’aide des états du projet. Configurez automatiquement les dossiers d’AEM en fonction des Portfolios, programmes et projets Workfront. | Non | ✓ | Non |
| Accès direct aux référentiels d’AEM à partir de Workfront | Permet aux utilisateurs d’accéder aux référentiels AEM disponibles configurés dans Workfront. | ✓ | Non | ✓ |
...
| Prise en charge de plusieurs environnements Workfront se connectant à un seul environnement AEM | Les utilisateurs de plusieurs environnements Workfront peuvent se connecter à un seul environnement AEM. | ✓ | Non | ✓ |
...
| Affichage en temps réel dans Workfront pour les métadonnées mappées à AEM | Affichez les métadonnées mappées mises à jour dans les AEM des panneaux Détails du document Workfront et Résumé du document. | ✓ | Non | ✓ |
| Push en temps réel des métadonnées Workfront mises à jour vers AEM | Mettez automatiquement à jour les métadonnées Workfront mappées vers AEM sans récupérer une ressource ou une nouvelle version d’une ressource. | ✓ | Non | ✓ |
