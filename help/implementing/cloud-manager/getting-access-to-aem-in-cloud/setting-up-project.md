...
* Le `<servers>` élément de ce fichier est fusionné dans la valeur par défaut `settings.xml` fichier fourni par Cloud Manager.
...
Dans Cloud Manager, chaque compilation peut produire un certain nombre de packages de contenu. Pour diverses raisons, il peut être préférable de produire un package de contenu, mais de ne pas le déployer, Par exemple, lors de la création de modules de contenu utilisés uniquement à des fins de test ou qui sont reconditionnés par une autre étape du processus de création. C&#39;est-à-dire un sous-paquet d&#39;un autre paquet.

Pour répondre à ces scénarios, Cloud Manager recherche une propriété nommée `cloudManagerTarget` dans les propriétés des modules de contenu créés. Si cette propriété est définie sur `none`, le module est ignoré et n’est pas déployé.
...
Si vous le souhaitez, le comportement de réutilisation peut être désactivé pour des pipelines spécifiques en définissant la variable de pipeline `CM_DISABLE_BUILD_REUSE` sur `true`. Si cette variable est définie, le hachage de validation est toujours extrait et les artefacts résultants sont stockés en vue d’une utilisation ultérieure, mais aucun artefact précédemment stocké ne sera réutilisé. Pour comprendre ce comportement, considérez le scénario suivant.
