...
>[!NOTE]
>
>L’agent de modernisation d’Experience remplace l’ancienne compétence de migration de l’[agent de production Experience.](/help/ai-in-aem/agents/production/overview.md)

## Présentation {#introduction}

L’agent de modernisation de l’expérience libère toute la valeur de Edge Delivery Services (y compris la création AEM) en rendant les migrations de sites web et l’évolution continue rapides et simples.

Il combine [compétences en création et migration de site](#creation-migration) pour l’intégration initiale du site web et [fonctionnalités de développement par blocs](#block-development) pour le développement continu de l’expérience (mises à jour de style, affinement des modèles, création de pages de destination). En outre, il propose la [console de modernisation de l’expérience](#console) en tant qu’environnement de développement hébergé assisté par l’IA, disponible directement pour vous. Bien que les utilisateurs puissent utiliser l’agent directement via cette console, les développeurs conservent un contrôle total sur les éléments livrés.

En outre, pour assurer la réussite des migrations complexes, Adobe propose le modèle de diffusion [AOE pour Agentic Outcome Engineer)](#delivery-model). Cette option peut être utilisée comme accélérateur ou comme service tactique pour aider à débloquer des défis spécifiques au projet.

## Avantages {#benefits}

L’agent de modernisation de l’expérience accélère le délai d’évaluation pour l’adoption de [Edge Delivery Services](/help/edge/overview.md) et vous permet d’adapter l’expérience web de votre marque.

* **Vitesse élevée** : l’automatisation de l’IA gère les tâches de migration répétitives (importation de contenu, mappage de blocs, application de système de conception), en comprimant les mois d’effort en semaines
* **Rentable** : l’automatisation gère les travaux répétitifs, libérant ainsi des services professionnels pour des tâches à forte valeur ajoutée telles que les intégrations et les décisions stratégiques
...
* **Valeur continue** : l’agent prend en charge l’évolution continue du site, y compris les mises à jour de style, l’amélioration des modèles et la création de pages de destination
...
* Transforme des sites web de n’importe quel CMS, AEM hérité ou système de conception (comme Figma) en projets Edge Delivery Services prêts pour la production
* Tient toutes les promesses de Edge Delivery Services : préparation de l’IA aux fonctionnalités agentiques, performances rapides (optimisées pour les composants Web vitaux principaux), accessibilité (WCAG 2.1 AA), conception réactive sur tous les points d’arrêt et agilité du contenu et du code
...
Experience Modernization Agent fournit un environnement de développement hébergé assisté par l’IA pour Edge Delivery Services, exposé sous forme d’interface web à l’adresse [`aemcoder.adobe.io`.](https://aemcoder.adobe.io)
...
* La gouvernance d’entreprise est mise en œuvre lorsque les développeurs conservent un contrôle total sur les éléments fournis via le processus habituel de révision et d’approbation GitHub.
...
L&#39;outil de grattage ne prend pas en charge les sources suivantes.
