...
#### Pour une intégration au composant de base Assets :{#feature-integration}
...
>Le [composant Fragment de contenu fait partie des composants de base[#$tu71]. Voir [Développement de composants de base[#$tu72] pour plus d’informations.
>
>
...
### Les fragments de contenu peuvent être intégrés à :{#integration-with-other-frameworks}
...
* **Les fragments de contenu sont entièrement intégrés au workflow de traduction AEM. Au niveau architectural, cela présente les implications suivantes :**
   ...
   * elles sont situées sous des racines de langue différentes, mais partagent exactement le même chemin relatif sous la racine de langue appropriée :
      ...
      * `/content/dam/<path>/en/<to>/<fragment>`
         ...
         `/content/dam/<path>/en/<to>/<fragment>`ou

         `/content/dam/<path>/de/<to>/<fragment>`

         `/content/dam/<path>/de/<to>/<fragment>`Outre les chemins basés sur des règles, il n’existe aucune connexion entre les différentes versions linguistiques d’un fragment de contenu ; elles sont traitées comme deux fragments distincts, bien que l’IU fournisse les moyens de naviguer entre les variantes de langues.
   * [!NOTE]
...
* Les fragments de contenu (ré)utilisent les [schémas de métadonnées](/help/assets/metadata-schemas.md) qui peuvent être définis par de ressources standard.
   ...
   * CFM fournit son propre schéma spécifique :[](/help/assets/metadata-schemas.md)

   * `/libs/dam/content/schemaeditors/forms/contentfragment`

      `/libs/dam/content/schemaeditors/forms/contentfragment`il peut être étendu si nécessaire.
...
## Vous pouvez utiliser l’API côté serveur pour accéder à vos fragments de contenu ; voir :{#the-content-fragment-management-api-server-side}

[com.adobe.cq.dam.cfm[#$tu111]



>[!CAUTION]Il est fortement conseillé d’utiliser l’API côté serveur plutôt que d’accéder directement à la structure du contenu.
>
>Interfaces principales  {#key-interfaces}

### Les trois interfaces suivantes peuvent faire office de points d’entrée :{#key-interfaces}

**Fragment de contenu** ([ContentFragment[#$tu117])
...
* 
   ...
   * Établissement de la liste des éléments
      ...
      * Accès aux données des éléments (voir `ContentElement`)](#caveats)

      * Établissement de la liste des variations définies pour le fragment`ContentElement`
   ...
   **Élément de contenu** ([ContentElement[#$tu136])
   ...
   * 
      ...
      * Établissement de la liste des variations
         ...
         * Raccourci pour résoudre les variations (en appliquant une certaine logique supplémentaire spécifique à la mise en œuvre si la variation spécifiée n’est pas disponible pour un élément)`ContentVariation`
      * **Variation de contenu** ([ContentVariation[#$tu147])
   * 
   ...
   Création d’une version de l’élément`ContentFragment``ContentElement``ContentVariation``Versionable`
...
### Ce qui suit peut être adapté :{#adapting-using-adaptto}
...
* `Resource` - ressource Sling sous-jacente ; la mise à jour directe de la `Resource` sous-jacente nécessite la reconstruction de l’objet `ContentFragment`.
   ...
   * `ContentElement` peut être adapté en :`Asset``Asset``ContentFragment`
...
### Il convient de noter les éléments suivants :{#caveats}
...
* Créez des variations de `ContentFragment` pour mettre la structure de données à jour.
   ...
   * L’API de gestion des fragments de contenu – côté client  {#the-content-fragment-management-api-client-side}`ContentFragment.removeVariation()`

## [!CAUTION]

>[!CAUTION]L’API côté client est interne.
>
>Informations supplémentaires {#additional-information}

### Reportez-vous aux informations suivantes :{#additional-information}
...
### Pour ce faire, vous pouvez adapter la ressource qui représente l’API à :{#example-accessing-an-existing-content-fragment}

`com.adobe.cq.dam.cfm.ContentFragment`

`com.adobe.cq.dam.cfm.ContentFragment`Par exemple :
...
Nœud : `<conf-root>/settings/dam/cfm/jcr:content`](/help/assets/content-fragments/content-fragments-managing.md#save-cancel-and-versions)
...
* Si vous souhaitez définir un intervalle d’enregistrement automatique de 5 minutes, vous devez définir la propriété sur le nœud, par exemple :`600``/libs/settings/dam/cfm/jcr:content`
...
## Pour plus d’informations, voir :{#components-for-page-authoring}

[Composants de base – composant de fragment de contenu[#$tu206] (recommandé)
