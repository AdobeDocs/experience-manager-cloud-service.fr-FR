...
1. La nouvelle carte du programme d’environnement de test s’affiche sur la page d’entrée et vous pouvez la survoler avec la souris pour sélectionner l’icône de Cloud Manager afin d’accéder à la page d’aperçu de Cloud Manager. La carte indiquera à l’utilisateur l’état de la configuration automatique du programme d’environnement de test nouvellement créé. L’utilisateur verra la progression.
...
1. Une fois le pipeline hors production qui se déploie vers le développement terminé, l’assistant guide l’utilisateur à accéder à AEM (en cours de développement) ou à déployer le code vers l’environnement de développement :

   ![](assets/create-program-setup-deploy.png)

   >[!NOTE]
   >Vous pouvez également modifier, changer ou ajouter un programme à partir de la page Présentation de Cloud Manager , comme illustré ci-dessous :
