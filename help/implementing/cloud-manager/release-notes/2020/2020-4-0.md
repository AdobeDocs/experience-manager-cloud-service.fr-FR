...
* Modifications apportées à la navigation pour permettre à l’utilisateur de modifier, de changer ou d’ajouter un programme à partir de la page d’aperçu de Cloud Manager.
* Les modifications permettent aux utilisateurs de modifier le programme directement à partir de la carte du programme sur la page de destination de Cloud Manager.
* Nouveau statut du pipeline **Exécution de pipeline** affiché au niveau de l’environnement auquel il est associé.
* Améliorations de la lisibilité de la page d’exécution de pipeline. Cette amélioration inclut l’affichage du nom et du type du pipeline (pipeline hors production uniquement), ainsi qu’un badge indiquant si l’état du pipeline est En cours/Annulé/Échec.
* Infobulles pour améliorer l’expérience utilisateur et la compréhension des raisons pour lesquelles le bouton Ajouter un programme/environnement est désactivé.
...
* Les étapes individuelles du processus de création d’environnement expiraient plus tôt que nécessaire, ce qui entraînait l’échec du processus.
...
* Les notifications CX Enterprise n’ont pas été reçues de manière cohérente.
