...
* **Amélioration de la validation du domaine (Interface d’utilisation)**
  ...
  Lors de l’ajout d’un domaine, Cloud Manager affiche une erreur s’il est déjà installé dans un compte Fastly : « *Domaine déjà installé dans Fastly. Supprimez-le avant de l’ajouter à Cloud Service.* »

## Programme Private Beta {#private-beta-program}

Pour obtenir un accès exclusif aux fonctionnalités à venir avant leur publication générale, participez au programme Private Beta de Cloud Manager.

L’opportunité bêta privée suivante est actuellement disponible :
...
>[!NOTE]
>
>Actuellement, les contrôles de qualité du code de demande d’extraction par défaut sont exclusifs aux référentiels hébergés sur GitHub, mais une mise à jour est en cours pour étendre cette fonctionnalité à d’autres fournisseurs Git.
...
* **Unicité appliquée pour les noms de transfert de port de réseau avancé**
  ...
  Cloud Manager applique désormais un nom unique pour les transferts de port de mise en réseau avancée. Auparavant, les noms en double étaient autorisés, ce qui entraînait des conflits. Ce correctif garantit que chaque entrée de transfert de port a un nom distinct, conformément aux bonnes pratiques en matière d’intégrité de la configuration réseau. <!-- CMGR-67082 -->
