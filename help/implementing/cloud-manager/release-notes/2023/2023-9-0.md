...
* Les utilisateurs peuvent désormais choisir d’inclure des tests de contrôle de l’expérience optimisés par Google Lighthouse dans des pipelines hors production et en pile complète.
...
Vous souhaitez tester le nouveau tableau de bord ? Commencez par envoyer un email à `aem-lighthouse-pilot@adobe.com` à partir de votre email associé à votre Adobe ID.
...
* Une erreur occasionnelle a été corrigée lorsque toutes les étapes de l’exécution d’un pipeline ont été marquées comme étant terminées. Cependant, l’état du pipeline était toujours en cours d’exécution, ce qui donnait l’impression d’un état bloqué.
