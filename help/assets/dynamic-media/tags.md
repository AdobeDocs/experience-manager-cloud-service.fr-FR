...
L’extension *Visionneuses Dynamic Media* pour Experience Platform Tags fonctionne avec les visionneuses Dynamic Media 5.13. Elle permet aux clients Adobe Analytics et Experience Platform Tags d’utiliser les événements et les données des visionneuses Dynamic Media dans leurs configurations de balises.
...
Une fois défini, un élément de données peut être utilisé dans d’autres emplacements de l’interface utilisateur Experience Platform Tags, à l’aide du widget de sélecteur d’éléments de données. L’**action Définir les variables** de l’extension Adobe Analytics dans une règle fait référence aux éléments de données définis pour le suivi des visionneuses Dynamic Media (voir ci-dessous).

Consultez [Éléments de données](https://experienceleague.adobe.com/en/docs/experience-platform/tags/ui/data-elements) dans le Guide de l’utilisateur Experience Platform Tags.
...
L’action **Envoyer la balise** doit suivre l’action **Définir les variables**. L’action *Envoyer la balise* envoie en fait des données au serveur de suivi Analytics. Les deux actions, *Définir les variables* et *Envoyer une balise*, proviennent de l’extension Adobe Analytics.

Voir [Règles](https://experienceleague.adobe.com/en/docs/experience-platform/tags/ui/rules) dans le Guide de l’utilisateur d’Experience Platform Tags.
...
1. Dans l’onglet **[!UICONTROL Éléments de données]** , définissez un `AssetName` d’élément de données qui fait référence `asset` paramètre de l’événement `LOAD` à partir de l’extension Visionneuses Dynamic Media.
...
1. Obtenez le code incorporé pour la bibliothèque Experience Platform Tags en vous connectant d’abord à Experience Platform Tags (voir [Configurer Experience Platform Tags](#configuring-adobe-launch-for-the-integration)).
1. Sélectionnez **[!UICONTROL Propriété]**, puis cliquez sur l’onglet **[!UICONTROL Environnements]**.
...
En outre, il existe une option pour désactiver explicitement l’intégration d’Experience Platform Tags dans la visionneuse en spécifiant `launch=0` paramètre dans la configuration de la visionneuse. La valeur par défaut de ce paramètre est `1`.
...
* **[!UICONTROL Type de données d’événement des visionneuses DM]** : une liste déroulante affiche tous les événements de visionneuse avec des arguments pris en charge par l’extension Visionneuses Dynamic Media, ainsi qu’un élément **[!UICONTROL COMMUN]** spécial. Un élément **[!UICONTROL COMMON]** représente une liste de paramètres d’événement communs à tous les types d’événements envoyés par les visionneuses.
* **[!UICONTROL Paramètre de suivi]** : argument de l’événement de visionneuse Dynamic Media sélectionné.

![image2019-7-22_12-5-46](assets/image2019-7-22_12-5-46.png)

Pour obtenir la liste des événements pris en charge par chaque type de visionneuse, reportez-vous au [ Guide de référence des visionneuses Dynamic Media ](https://experienceleague.adobe.com/en/docs/dynamic-media-developer-resources/library/viewers-aem-assets-dmc/c-html5-s7-aem-asset-viewers), accédez à la section spécifique à la visionneuse, puis sélectionnez la sous-section Prise en charge du suivi d’Adobe Analytics. À l’heure actuelle, le guide de référence des visionneuses Dynamic Media ne documente pas les arguments d’événement.
...
La valeur exacte renvoyée par l’élément de données dépend du contexte. Si une règle déclenchée par un événement de visionneuse Dynamic Media demande l’élément de données, la valeur est renvoyée pour la visionneuse qui a initié la règle. Si une règle déclenchée par un événement provenant d’une autre extension Experience Platform Tags demande l’élément de données, elle suit le contexte de l’événement correspondant. À ce stade, la valeur de l’élément de données provient de la dernière visionneuse à mettre à jour cet élément de données.
...
Supposons à présent que l’utilisateur charge la page web avec les deux visionneuses. Dans la *visionneuse1*, il effectue un zoom avant à une échelle de 50 % ; ensuite, dans la *visionneuse2*, il effectue un zoom avant à une échelle de 25 %. Dans la *visionneuse1*, il effectue un panoramique sur l’image et appuie sur une touche du clavier.

En raison de l’activité de l’utilisateur, les deux appels de suivi suivants sont effectués vers Adobe Analytics :

* Le premier appel se produit, car la règle **[!UICONTROL TrackPan]** est déclenchée lorsque l’utilisateur effectue un panoramique dans *visionneuse1*. Cet appel envoie **50 %** comme valeur de l’élément de données **[!UICONTROL ZoomScale]**, car il reconnaît que *viewer1* a déclenché la règle et récupère la valeur d’échelle correspondante.
* Le deuxième appel se produit, car la règle **[!UICONTROL TrackKey]** est déclenchée lorsque l’utilisateur appuie sur une touche du clavier. Cet appel envoie 25 % en tant que valeur de l’élément de données **[!UICONTROL ZoomScale]**, car la visionneuse n’a pas déclenché la règle. L’élément de données renvoie donc la valeur la plus récente.

L’exemple de configuration ci-dessus affecte également la durée de vie de la valeur de l’élément de données. La valeur de l’élément de données géré par la visionneuse Dynamic Media est stockée dans le code de bibliothèque Experience Platform Tags, même après la suppression de la visionneuse de la page web. Cette fonctionnalité signifie que si une règle déclenchée par une extension de visionneuse autre que Dynamic Media référence l’élément de données, elle renvoie la dernière valeur connue. Même si la visionneuse n’est plus présente sur la page web.
...
Lorsque cet élément est sélectionné, l’éditeur d’événements affiche le menu déroulant **[!UICONTROL Événements de visionneuse Dynamic Media]** qui répertorie tous les événements pris en charge par les visionneuses Dynamic Media.
...
* Enfin, dans l’éditeur Définir les variables de l’extension Adobe Analytics, cliquez sur ![icône de données, sélecteur d’éléments de données](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Data_18_N.svg) **sélecteur d’éléments de données** pour ouvrir la boîte de dialogue Sélectionner un élément de données, puis cliquez sur un élément de données à partir de celle-ci.

![image2019-7-10_20-41-52](assets/image2019-7-10_20-41-52.png)

Il est toutefois possible d’utiliser une autre approche et d’éviter la création d’un élément de données. Vous pouvez référencer directement un argument à partir d’un événement de visionneuse Dynamic Media. Entrez le nom complet de l’argument d’événement dans le champ d’entrée **[!UICONTROL valeur]** de l’affectation de variable Analytics. Assurez-vous que les signes de pourcentage (%) sont entourés. Par exemple,
...
Par exemple, si l’événement **[!UICONTROL LOAD]** de l’extension Visionneuse Dynamic Media déclenche la règle, le référencement `%event.detail.dm.LOAD.asset%` renvoie le nom de ressource correct.

Cependant, elle renvoie une valeur vide pour tout autre événement.
...
Une fois Adobe Analytics configuré, l’intégration est configurée pour les éléments suivants :
...
1. Désignez une ou plusieurs variables Adobe Analytics à utiliser pour effectuer le suivi du comportement des visionneuses Dynamic Media sur la page web.
   ...
   Vous pouvez utiliser n’importe quel type de variable pris en charge par Adobe Analytics. Votre implémentation Analytics doit déterminer le type de variable approprié, tel que le trafic personnalisé (`props`) ou la conversion (`eVar`).
...
1. Sur la page **[!UICONTROL Gestionnaire de suites de rapports]**, sélectionnez le rapport approprié.
1. Dans la barre d’outils, cliquez sur **[!UICONTROL Modifier les paramètres]** > **[!UICONTROL Trafic]** > **[!UICONTROL Variables de trafic]**.
1. Sélectionnez une variable inutilisée, attribuez-lui un nom explicite (**[!UICONTROL ressource de visionneuse (prop 30)]**), puis redéfinissez la zone de liste modifiable sur « Activé » dans la colonne Activé.
...
En règle générale, les besoins spécifiques à un projet déterminent la manière dont vous configurez un rapport dans Adobe Analytics. Par conséquent, une configuration de rapport détaillée dépasse la portée de cette intégration.
...
Voir aussi [Configurer une propriété de sélection](https://experienceleague.adobe.com/en/docs/platform-learn/implement-mobile-sdk/initial-configuration/configure-tags).

**Pour créer une propriété dans Experience Platform Tags :**

1. Dans Experience Platform Tags, cliquez sur **[!UICONTROL Nouvelle propriété]**.
...
Voir [Extension du service d’identités Experience Cloud](https://experienceleague.adobe.com/en/docs/experience-platform/tags/extensions/client/id-service/overview).
...
Voir [Extension Adobe Analytics](https://experienceleague.adobe.com/en/docs/experience-platform/tags/extensions/client/analytics/overview).
...
* Experience Manager exécute les instances en mode création et en mode publication.
...
1. Dans l’auteur Experience Manager, cliquez sur ![Icône Marteau, outils](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Hammer_18_N.svg) **Outils**, puis accédez à **[!UICONTROL Sécurité]** > **[!UICONTROL Configurations d’Adobe IMS]**.

   ![2019-07-25_11-52-58](assets/2019-07-25_11-52-58.png)

1. Sur la page Configuration d’Adobe IMC, près du coin supérieur gauche, cliquez sur **[!UICONTROL Créer]**.
1. Sur la page **[!UICONTROL Configuration du compte technique Adobe IMS]**, dans la liste déroulante **[!UICONTROL Solution cloud]**, cliquez sur **[!UICONTROL Collecte de données Experience Platform]**.
...
1. Sélectionnez **[!UICONTROL Télécharger la clé publique]** pour télécharger un fichier de clé publique (`*.crt`) sur votre système local.
   ...
   >[!NOTE]
   >
   >À ce stade, ***laissez ouverte*** la page **[!UICONTROL Configuration du compte technique Adobe IMS]** ; ***ne fermez pas*** la page et ***ne le faites pas*** cliquez sur **[!UICONTROL Suivant]**. Vous y reviendrez plus tard.
...
1. Sur la page **[!UICONTROL Intégrations Adobe Developer Console]**, près du coin supérieur droit, cliquez sur **[!UICONTROL Nouvelle intégration]**.
...
1. Sur la deuxième page **[!UICONTROL Créer une intégration]**, activez le bouton radio **[!UICONTROL API Experience Platform Tags]**. Dans le coin inférieur droit de la page, cliquez sur **[!UICONTROL Continuer]**.
...
1. Sur la troisième page **[!UICONTROL Créer une intégration]**, procédez comme suit :
   ...
   * Sous l’en-tête **[!UICONTROL Sélectionner un rôle pour l’API Experience Platform Tags]**, cliquez sur **[!UICONTROL Admin]**.
...
1. Revenez à la page **[!UICONTROL Configuration du compte technique Adobe IMS]** que vous avez laissée ouverte précédemment. Dans le coin supérieur droit de la page, cliquez sur **[!UICONTROL Suivant]** pour ouvrir la page **[!UICONTROL Compte]** dans la fenêtre **[!UICONTROL Configuration du compte technique Adobe IMS]**.
   ...
   (Si vous avez déjà fermé la page, revenez à l’auteur Experience Manager puis accédez à **[!UICONTROL Outils]** > **[!UICONTROL Sécurité]** > **[!UICONTROL Configurations d’Adobe IMS]**. Sélectionnez **[!UICONTROL Créer]**. Dans la liste déroulante **[!UICONTROL Solution cloud]**, cliquez sur **[!UICONTROL Experience Platform Tags]**. Dans la liste déroulante **[!UICONTROL Certificat]**, cliquez sur le nom du certificat créé précédemment.
...
1. **[!UICONTROL Clé API]** : revenez à la page Informations concernant l’intégration. Sélectionnez l’onglet **[!UICONTROL Aperçu]** puis, à droite du champ **[!UICONTROL Clé API (ID client)]** cliquez sur **[!UICONTROL Copier]**.
...
1. En mode de création Experience Manager, près du coin supérieur gauche, cliquez sur ![Icône Marteau, outils](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Hammer_18_N.svg) **Outils**, puis accédez à **[!UICONTROL Services cloud]** > **[!UICONTROL Configurations d’Experience Platform Tags]**.
...
1. Sur la page **[!UICONTROL Évaluation]** (page 2/3) de la fenêtre **[!UICONTROL Créer une configuration Experience Platform Tags]**, renseignez les champs suivants :
   ...
   >[!NOTE]
   >
   >Vérifiez que l’URI de bibliothèque (Uniform Resource Identifier) auto-renseigné est correctement formé. Si nécessaire, corrigez-le de sorte qu’il soit relatif au protocole. C’est-à-dire qu’il commence par une double barre oblique.
   >
   >
...
1. Sélectionnez votre nouvelle configuration du cloud Experience Platform Tags (une coche apparaît à gauche du titre de la configuration lorsqu’elle est sélectionnée). Dans la barre d’outils, cliquez sur **[!UICONTROL Publier]**.
...
Il est possible de contourner cette limitation. Spécifiez l’URL de développement ou d’évaluation de la bibliothèque Experience Platform Tags dans la Configuration cloud Experience Platform Tags pour la publication Experience Manager ci-dessus. Ainsi, le nœud de publication Experience Manager utilise la version de développement ou d’évaluation de la bibliothèque Experience Platform Tags.

Consultez [Intégration d’Experience Platform Tags et d’Experience Manager](https://experienceleague.adobe.com/en/docs/experience-manager-learn/sites/integrations/experience-platform-data-collection-tags/overview#integrations) pour plus d’informations sur la configuration du Cloud Experience Platform Tags.
