---
title: Intégration du gestionnaire d’accès à l’API ouverte Dynamic Media
description: Intégrez le gestionnaire de contenu à diverses applications Adobe, non Adobe et tierces.
role: Admin, User
...
# Intégration de Dynamic Media aux fonctionnalités OpenAPI {#integrate-dynamic-media-open-apis}

Le gestionnaire d’accès vous permet de réaliser une intégration à l’aide de diverses applications Adobe afin de leur permettre de travailler ensemble en toute transparence.

## Prérequis {#prereqs-polaris}

Utilisez les conditions préalables suivantes si vous intégrez Content Advisor à Dynamic Media avec des fonctionnalités OpenAPI :

* [Méthodes de communication](/help/assets/content-advisor-properties.md#prereqs)
...
L’intégration du gestionnaire d’accès au processus OpenAPI Dynamic Media implique plusieurs étapes, dont la création d’une URL Dynamic Media personnalisée ou l’utilisation d’une URL Dynamic Media prête à être sélectionnée, etc.

### Intégration de la fonction de conseil en contenu pour Dynamic Media aux fonctionnalités OpenAPI {#integrate-dynamic-media}
...
Cette configuration vous permet d’afficher toutes les ressources approuvées sans dossiers ou sous la forme d’une structure plate. Pour plus d’informations, accédez à `aemTierType` propriété sous [Propriétés du gestionnaire d’accès](/help/assets/content-advisor-properties.md).
...
Par exemple, dans le gestionnaire d’accès Dynamic Media avec fonctionnalités OpenAPI, une propriété sur `asset jcr:content/metadata/client_name:market` est convertie en `metadata.embedded.client_name:market` pour la configuration des filtres.
